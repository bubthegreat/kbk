...
void spell_ball_lightning(int sn,int level,CHAR_DATA *ch,void *vo, int target)
{
	...
	if (victim == ch) return send_to_char("Logic prevails and prevents you from doing that.\n\r",ch);
	...
	for (rch = victim->in_room->people; rch != NULL; rch = rch_next) {
		...
		if (is_same_group(rch,victim) && rch != victim) {
			...
			dam = offtarget_dam;
			if (saves_spell(level,rch,DAM_LIGHTNING)) dam *= 0.66;
...
void spell_group_sanctuary(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	if (((IS_NPC(ch) && IS_NPC(gch)) ||
	    (!IS_NPC(ch) && !IS_NPC(gch))) && is_same_group(ch,gch) && !IS_AFFECTED(gch,AFF_SANCTUARY)
	&& !is_affected(gch,skill_lookup("sanctuary")))
	{
...
void spell_resurrect(int sn, int level, CHAR_DATA *ch, void *vo, int target) {
	...
	if (!dead_name) return send_to_char("They don't seem to exist.\n\r",ch);
	
	if (corpse->timer < 2) return send_to_char("That corpse will decay long before the ritual is complete.\n\r",ch);
	
	for (search = char_list; search != NULL; search = search->next)
	{
	  	if (!IS_NPC(search) && !str_cmp(dead_name,search->original_name)) {
		
			if (search == ch) return send_to_char("Resurrect yourself?\n\r",ch);
			if (search->ghost == 0) return send_to_char("They have no connection to the afterlife.\n\r",ch);
			if (search->ghost < 2) return send_to_char("Their connection to the afterlife grows too tenuous.\n\r",ch);
...
void spell_divine_transferrance(int sn, int level, CHAR_DATA *ch, void *vo, int target) {
	...
	if (ch == victim) return send_to_char("You can only cast that on others.\n\r",ch);
...
void spell_maelstrom(int sn, int level, CHAR_DATA *ch, void *vo, int target) {
	...
	if (is_affected(victim,skill_lookup("maelstrom"))) return send_to_char("They are already affected by the elemental maelstrom.\n\r",ch);
...
void spell_hurricane(int sn, int level, CHAR_DATA *ch, void *vo, int target) {
	...
	if (IS_NPC(victim)) return send_to_char("Their movements are too erratic for you to summon a hurricane around.\n\r",ch);
	
	if (ch == victim) return send_to_char("Something tells you that would be a bad idea.\n\r",ch);
	
	if (is_affected(ch,skill_lookup("hurricane"))) return send_to_char("You aren't ready to summon another hurricane.\n\r",ch);
	
	if (is_affected_room(ch->in_room,skill_lookup("hurricane"))) return send_to_char("There is already a hurricane here.\n\r",ch);
	...
	if (!exitsOpen) return send_to_char("You have no room to form a hurricane here.\n\r",ch);
	...
	for (rch = ch->in_room->people; rch != NULL; rch = rch_next ) {
		...
		if (rch == ch) continue;
		if (rch == victim) continue;
		if (IS_IMMORTAL(rch)) continue;
			...
			for (exits = number_range(0,5); !thrownOut && throw_attempts < throw_attempts_max; exits = number_range(0,5)) {
				...
				if (ex) {
					if (!IS_SET(ex->exit_info,EX_CLOSED)) {
						...
						switch (exits) {
							case DIR_NORTH: do_north(rch, ""); if (rch->in_room->vnum != was_in) thrownOut = TRUE; else break;
											...
											act("You are violently blown out of the room by the billowing winds!",rch,0,0,TO_CHAR); break;
							case DIR_EAST:	do_east(rch, ""); if (rch->in_room->vnum != was_in) thrownOut = TRUE; else break;
											act("$N is violently blown out of the room by the billowing winds!",ch,0,rch,TO_ROOM);
											act("$N is violently blown out of the room by the billowing winds!",ch,0,rch,TO_CHAR);
											act("You are violently blown out of the room by the billowing winds!",rch,0,0,TO_CHAR); break;
							case DIR_WEST:	do_west(rch, ""); if (rch->in_room->vnum != was_in) thrownOut = TRUE; else break;
											act("$N is violently blown out of the room by the billowing winds!",ch,0,rch,TO_ROOM); 
											act("$N is violently blown out of the room by the billowing winds!",ch,0,rch,TO_CHAR);
											act("You are violently blown out of the room by the billowing winds!",rch,0,0,TO_CHAR); break;
							case DIR_SOUTH:	do_south(rch, ""); if (rch->in_room->vnum != was_in) thrownOut = TRUE; else break;
											act("$N is violently blown out of the room by the billowing winds!",ch,0,rch,TO_ROOM); 
											act("$N is violently blown out of the room by the billowing winds!",ch,0,rch,TO_CHAR);
											act("You are violently blown out of the room by the billowing winds!",rch,0,0,TO_CHAR); break;
							case DIR_DOWN:	do_down(rch, ""); if (rch->in_room->vnum != was_in) thrownOut = TRUE; else break;
											act("$N is violently blown out of the room by the billowing winds!",ch,0,rch,TO_ROOM); 
											act("$N is violently blown out of the room by the billowing winds!",ch,0,rch,TO_CHAR);
											act("You are violently blown out of the room by the billowing winds!",rch,0,0,TO_CHAR); break;
							case DIR_UP:	do_up(rch, ""); if (rch->in_room->vnum != was_in) thrownOut = TRUE; else break;
											act("$N is violently blown out of the room by the billowing winds!",ch,0,rch,TO_ROOM); 
											act("$N is violently blown out of the room by the billowing winds!",ch,0,rch,TO_CHAR);
											act("You are violently blown out of the room by the billowing winds!",rch,0,0,TO_CHAR); break;
						}
...
void spell_vorpal_bolt(int sn, int level, CHAR_DATA *ch, void *vo, int target) {
	...
	switch (type) {
		case 0: if (!is_affected(victim,skill_lookup("vorpal bolt head"))) found = FALSE; gsn = skill_lookup("vorpal bolt head"); break;
		case 1: if (!is_affected(victim,skill_lookup("vorpal bolt leg"))) found = FALSE; gsn = skill_lookup("vorpal bolt leg"); break;
		case 2: if (!is_affected(victim,skill_lookup("vorpal bolt arm"))) found = FALSE; gsn = skill_lookup("vorpal bolt arm"); break;
		case 3: if (!is_affected(victim,skill_lookup("vorpal bolt chest"))) found = FALSE; gsn = skill_lookup("vorpal bolt chest"); break;
	}
...
void spell_ward_of_magnetism(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	if (is_affected(ch, sn)) return send_to_char("You are already surrounded by a field of electromagnetic energy.\n\r", ch);
