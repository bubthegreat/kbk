...
bool is_friend(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    if (IS_SET(ch->off_flags, ASSIST_ALIGN) && !IS_SET(ch->act, ACT_NOALIGN) && !IS_SET(victim->act, ACT_NOALIGN) && ((IS_GOOD(ch) && IS_GOOD(victim)) || (IS_EVIL(ch) && IS_EVIL(victim)) || (IS_NEUTRAL(ch) && IS_NEUTRAL(victim))))
        return TRUE;
...
int get_skill(CHAR_DATA *ch, int sn)
{
    ...
    else /* mobiles */
    {
        ...
        else if ((sn == gsn_dodge && IS_SET(ch->off_flags, OFF_DODGE)) || (sn == gsn_parry && IS_SET(ch->off_flags, OFF_PARRY)))
            skill = ch->level * 2;
        ...
        else if (sn == gsn_enhanced_damage_two && IS_SET(ch->act, ACT_WARRIOR))
        {
            if (ch->level < 35)
                skill = 0;
            else
        ...
        else if (sn == gsn_disarm && (IS_SET(ch->off_flags, OFF_DISARM) || IS_SET(ch->act, ACT_WARRIOR) || IS_SET(ch->act, ACT_THIEF)))
            skill = 20 + 3 * ch->level;
        ...
        else if (sn == gsn_sword || sn == gsn_dagger || sn == gsn_spear || sn == gsn_mace || sn == gsn_axe || sn == gsn_flail || sn == gsn_whip || sn == gsn_polearm)
...
void reset_char(CHAR_DATA *ch)
{
    ...
    if (ch->pcdata->perm_hit == 0 || ch->pcdata->perm_mana == 0 || ch->pcdata->perm_move == 0 || ch->pcdata->last_level == 0)
    {
        /* do a FULL reset */
        ...
        if (!obj->enchanted)
            for (af = obj->pIndexData->affected; af != NULL; af = af->next)
            {
                ...
                case APPLY_DAM_MOD:
                    ch->dam_mod += mod;
                    break;
                case APPLY_ATTACK_MOD:
                    ch->numAttacks += mod;
                    break;
                case APPLY_ENHANCED_MOD:
                    ch->enhancedDamMod += mod;
                    break;
                case APPLY_SIZE:
                    ch->size += mod;
                    break;
...
int get_curr_stat(CHAR_DATA *ch, int stat)
{
    ...
    else
    {
        ...
        {
            if ((iClass == GUILD_WARRIOR || iClass == GUILD_PALADIN || iClass == GUILD_ANTI_PALADIN || iClass == GUILD_RANGER) && stat == STAT_STR)
                max = 23;
            else if ((iClass == GUILD_THIEF || iClass == GUILD_DRUID ||
                      iClass == GUILD_ASSASSIN) &&
                     stat == STAT_DEX)
                max = 23;
            else if ((iClass == GUILD_CLERIC && stat == STAT_WIS))
                max = 23;
            else if ((iClass == GUILD_CHANNELER || iClass == GUILD_NECROMANCER || iClass == GUILD_ELEMENTALIST) && stat == STAT_INT)
                max = 23;
        }
...
int get_max_train(CHAR_DATA *ch, int stat)
{
    ...
    if (!str_cmp(race_table[ch->race].name, "human"))
    {
        if ((iClass == GUILD_WARRIOR || iClass == GUILD_PALADIN || iClass == GUILD_ANTI_PALADIN || iClass == GUILD_RANGER) && stat == STAT_STR)
            max = 23;
        else if ((iClass == GUILD_THIEF || iClass == GUILD_DRUID || iClass == GUILD_ASSASSIN) && stat == STAT_DEX)
            max = 23;
        else if ((iClass == GUILD_CLERIC && stat == STAT_WIS))
            max = 23;
        else if ((iClass == GUILD_CHANNELER || iClass == GUILD_NECROMANCER || iClass == GUILD_ELEMENTALIST) && stat == STAT_INT)
...
bool can_pk(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    if (IS_NPC(ch) || IS_NPC(victim) || (ch->ghost > 0) || (victim->ghost > 0) || (ch->level < MIN_LEVEL_TO_PK) || (victim->level < MIN_LEVEL_TO_PK))
        return FALSE;
...
void char_from_room(CHAR_DATA *ch)
{
    ...
    for (obj = ch->carrying; obj != NULL; obj = obj->next_content)
    {
        ...
        {
            if ((obj->item_type == ITEM_LIGHT || (obj->item_type == ITEM_WEAPON && IS_OBJ_STAT(obj, ITEM_GLOW))) && ch->in_room != NULL && ch->in_room->light > 2)
                ch->in_room->light -= 3;
            else if (IS_OBJ_STAT(obj, ITEM_GLOW) && ch->in_room != NULL && ch->in_room->light > 0)
                ch->in_room->light -= 3;
        }
    }
...
void char_to_room(CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex)
{
    ...
    for (obj = ch->carrying; obj != NULL; obj = obj->next_content)
    {
        if (obj->wear_loc != WEAR_NONE)
        {
            if ((obj->item_type == ITEM_LIGHT || (obj->item_type == ITEM_WEAPON && IS_OBJ_STAT(obj, ITEM_GLOW))) && ch->in_room != NULL)
                ch->in_room->light += 3;
            else if (IS_OBJ_STAT(obj, ITEM_GLOW) && ch->in_room != NULL)
                ch->in_room->light += 3;
        }
    }
    ...
    if (IS_AFFECTED(ch, AFF_PLAGUE))
    {
        ...
        {
            if (!saves_spell(plague.level - 2, vch, DAM_DISEASE) && !IS_IMMORTAL(vch) &&
...
void equip_char(CHAR_DATA *ch, OBJ_DATA *obj, int iWear)
{
    ...
    if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_GOOD) && IS_GOOD(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_NEUTRAL) && IS_NEUTRAL(ch)) || is_restricted(ch, obj))
    {
    ...
    if (!obj->enchanted)
        for (paf = obj->pIndexData->affected; paf != NULL; paf = paf->next)
        {
            ...
            if (paf->bitvector)
            {
                ...
                {
                case TO_AFFECTS:
                    sprintf(buf, "You are now affected by %s.\n\r", affect_bit_name(paf->bitvector));
                    break;
                case TO_IMMUNE:
                    sprintf(buf, "Your body builds an immunity to %s.\n\r", imm_bit_name(paf->bitvector));
                    break;
                case TO_RESIST:
                    sprintf(buf, "Your body feels resistant to %s.\n\r", imm_bit_name(paf->bitvector));
                    break;
                case TO_VULN:
                    sprintf(buf, "Your body feels vulnerable to %s.\n\r", imm_bit_name(paf->bitvector));
                    break;
                }
                send_to_char(buf, ch);
            }
    ...
    if (!obj->enchanted)
        for (paf = obj->pIndexData->affected; paf != NULL; paf = paf->next)
        {
            ...
            if (paf->bitvector)
            {
                ...
                {
                case TO_AFFECTS:
                    sprintf(buf, "You are no longer affected by %s.\n\r", affect_bit_name(paf->bitvector));
                    break;
                case TO_IMMUNE:
                    sprintf(buf, "Your immunity to %s seems to fade away.\n\r", imm_bit_name(paf->bitvector));
                    break;
                case TO_RESIST:
                    sprintf(buf, "You feel less resistant to %s.\n\r", imm_bit_name(paf->bitvector));
                    break;
                case TO_VULN:
                    sprintf(buf, "Your body is no longer so vulnerable to %s.\n\r", imm_bit_name(paf->bitvector));
                    break;
                }
...
int get_obj_number(OBJ_DATA *obj)
{
    ...
    if (obj->wear_loc == WEAR_BRAND || obj->item_type == ITEM_MONEY || obj->item_type == ITEM_GEM || obj->item_type == ITEM_JEWELRY)
        number = 0;
    else
        number = 1;
...
bool can_see_room(CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex)
{
    ...
    if (IS_SET(pRoomIndex->room_flags, ROOM_NEWBIES_ONLY) && !IS_NPC(ch) && !isNewbie(ch) && !IS_IMMORTAL(ch))
        return FALSE;

    if (IS_SET(pRoomIndex->room_flags, ROOM_LOW_ONLY) && ch->level > 10 && !IS_SET(ch->in_room->room_flags, ROOM_LOW_ONLY))
        return FALSE;

    if (IS_SET(pRoomIndex->room_flags, ROOM_NO_NEWBIE) && isNewbie(ch))
        return FALSE;
...
bool can_see(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    if ((is_affected(victim, gsn_earthfade) && (!is_affected(ch, gsn_acute_vision) || (ch->class != CLASS_ELEMENTALIST))))
        return FALSE;

    if (is_affected(victim, gsn_duo))
        return FALSE;

    if (IS_NPC(ch) && ch->pIndexData->vnum == MOB_VNUM_ENFORCER_G && IS_SET(victim->act, PLR_CRIMINAL))
        return TRUE;

    if (IS_AFFECTED(victim, AFF_INVISIBLE) && !IS_AFFECTED(ch, AFF_DETECT_INVIS))
        return FALSE;

    if ((IS_AFFECTED(victim, AFF_HIDE) && !IS_AFFECTED(ch, AFF_DETECT_HIDDEN)) || (victim->fighting == NULL && (get_skill(victim, skill_lookup("veil of the opaque")) == 100 && get_skill(ch, skill_lookup("veil of the opaque")) < 100)))
        return FALSE;

    if (IS_AFFECTED(victim, AFF_CAMOUFLAGE) && !is_affected(ch, gsn_acute_vision))
        return FALSE;

    if (is_affected(victim, gsn_forest_blending) && !is_affected(ch, gsn_awareness))
    {
...
char *extra_bit_name(int extra_flags)
{
    ...
    if (extra_flags & ITEM_HIDDEN)
        strcat(buf, " hidden");
    if (extra_flags & ITEM_ANTI_NEUT)
        strcat(buf, " anti-neut");
    if (extra_flags & ITEM_ANTI_CHAOTIC)
    ...
    if (extra_flags & ITEM_BRAND)
        strcat(buf, " branded");
    if (extra_flags & ITEM_METAL)
        strcat(buf, " metal");
    if (extra_flags & ITEM_FIXED)
        strcat(buf, " fixed");
    return (buf[0] != '\0') ? buf + 1 : "none";
}
...
char *restrict_bit_name(restrict_flags)
{
    ...
    buf[0] = '\0';
    if (restrict_flags & HUMAN_ONLY)
        strcat(buf, " human_only");
    if (restrict_flags & DWARF_ONLY)
        strcat(buf, " dwarf_only");
    if (restrict_flags & ELF_ONLY)
        strcat(buf, " elf_only");
    if (restrict_flags & DARK_ONLY)
        strcat(buf, " drow_only");
    if (restrict_flags & GIANT_ONLY)
        strcat(buf, " giant_only");
    if (restrict_flags & CENTAUR_ONLY)
        strcat(buf, " centaur_only");
    if (restrict_flags & TROLL_ONLY)
        strcat(buf, " troll_only");
    if (restrict_flags & DRACONIAN_ONLY)
        strcat(buf, " draconian_only");
    if (restrict_flags & GNOME_ONLY)
        strcat(buf, " gnome_only");
    if (restrict_flags & WARRIOR_ONLY)
        strcat(buf, " warrior_only");
    if (restrict_flags & THIEF_ONLY)
        strcat(buf, " thief_only");
    if (restrict_flags & ZEALOT_ONLY)
        strcat(buf, " zealot_only");
    if (restrict_flags & PALADIN_ONLY)
        strcat(buf, " paladin_only");
    if (restrict_flags & ANTI_PALADIN_ONLY)
        strcat(buf, " AP_only");
    if (restrict_flags & BARD_ONLY)
        strcat(buf, " bard_only");
    if (restrict_flags & RANGER_ONLY)
        strcat(buf, " ranger_only");
    if (restrict_flags & CHANNELER_ONLY)
        strcat(buf, " channeler_only");
    if (restrict_flags & ASSASSIN_ONLY)
        strcat(buf, "assassin_only");
    if (restrict_flags & NECROMANCER_ONLY)
        strcat(buf, " necromancer_only");
    if (restrict_flags & ELEMENTALIST_ONLY)
        strcat(buf, " elementalist_only");
    if (restrict_flags & HEALER_ONLY)
        strcat(buf, " healer_only");
    if (restrict_flags & RAGER_ONLY)
        strcat(buf, " rager_only");
    if (restrict_flags & ANCIENT_ONLY)
        strcat(buf, " ancient_only");
    if (restrict_flags & MAGE_ONLY)
        strcat(buf, " mage_only");
    if (restrict_flags & ARCANA_ONLY)
        strcat(buf, " arcana_only");
    if (restrict_flags & KNIGHT_ONLY)
        strcat(buf, " knight_only");
    if (restrict_flags & OUTLAW_ONLY)
        strcat(buf, " outlaw_only");
    if (restrict_flags & BOUNTY_ONLY)
        strcat(buf, " bounty_only");
    if (restrict_flags & NEWBIE_ONLY)
        strcat(buf, " newbie_only");
    return (buf[0] != '\0') ? buf + 1 : "none";
}
...
char *act_bit_name(int act_flags)
{
    ...
    if (IS_SET(act_flags, ACT_IS_NPC))
    {
        ...
        if (act_flags & ACT_BANKER)
            strcat(buf, " banker");
        if (act_flags & ACT_NO_TRACK)
            strcat(buf, " no_track");
        if (act_flags & ACT_INNER_GUARDIAN)
            strcat(buf, " inner_guardian");
        if (act_flags & ACT_OUTER_GUARDIAN)
            strcat(buf, " outer_guardian");
    }
    else
    {
        strcat(buf, " player");
        ...
        if (act_flags & PLR_KILLER)
            strcat(buf, " killer");
        if (act_flags & PLR_CRIMINAL)
            strcat(buf, " criminal");
        if (act_flags & PLR_HEROIMM)
            strcat(buf, " heroimm");
        if (act_flags & PLR_COLOR)
            strcat(buf, " color");
        if (act_flags & PLR_EMPOWERED)
            strcat(buf, " empowered");
        if (act_flags & PLR_MORON)
            strcat(buf, " moron");
    }
    return (buf[0] != '\0') ? buf + 1 : "none";
}
...
char *wear_bit_name(int wear_flags)
{
    ...
    if (wear_flags & ITEM_NO_SAC)
        strcat(buf, " nosac");
    if (wear_flags & ITEM_WEAR_BRAND)
        strcat(buf, " brand");
    if (wear_flags & ITEM_WEAR_STRAPPED)
        strcat(buf, " strapped");
...
char *sector_bit_name(int sector_flags)
{
    static char buf[512];
    buf[0] = '\0';
    if (sector_flags == SECT_INSIDE)
        strcat(buf, " inside");
    if (sector_flags == SECT_CITY)
        strcat(buf, " city");
    if (sector_flags == SECT_FIELD)
        strcat(buf, " field");
    if (sector_flags == SECT_FOREST)
        strcat(buf, " forest");
    if (sector_flags == SECT_HILLS)
        strcat(buf, " hills");
    if (sector_flags == SECT_MOUNTAIN)
        strcat(buf, " mountain");
    if (sector_flags == SECT_WATER_SWIM)
        strcat(buf, " water_swim");
    if (sector_flags == SECT_WATER_NOSWIM)
        strcat(buf, " water_noswim");
    if (sector_flags == SECT_UNUSED)
        strcat(buf, " unused");
    if (sector_flags == SECT_AIR)
        strcat(buf, " air");
    if (sector_flags == SECT_DESERT)
        strcat(buf, " desert");
    if (sector_flags == SECT_ROAD)
        strcat(buf, " road");
    if (sector_flags == SECT_UNDERWATER)
        strcat(buf, " underwater");
    if (sector_flags == SECT_MAX)
        strcat(buf, " max");
    return (buf[0] != '\0') ? buf + 1 : "none";
}
...
char *off_bit_name(int off_flags)
{
    ...
    if (off_flags & STATIC_TRACKING)
        strcat(buf, " static_track");
    if (off_flags & SPAM_MURDER)
        strcat(buf, " spam_murder");
    if (off_flags & OFF_ATTACK_GOOD)
        strcat(buf, " attack_good");
    if (off_flags & OFF_ATTACK_EVIL)
        strcat(buf, " attack_evil");
    return (buf[0] != '\0') ? buf + 1 : "none";
}
...
char *raffect_bit_name(int vector)
{
    ...
    buf[0] = '\0';
    if (vector & AFF_ROOM_CURSE)
        strcat(buf, " curse");
    if (vector & AFF_ROOM_POISON)
        strcat(buf, " poison");
    if (vector & AFF_ROOM_PLAGUE)
        strcat(buf, " plague");
    if (vector & AFF_ROOM_SLEEP)
        strcat(buf, " sleep");
    if (vector & AFF_ROOM_SLOW)
        strcat(buf, " slow");
    if (vector & AFF_ROOM_TRIPWIRE)
        strcat(buf, " tripwire");
    return (buf[0] != '\0') ? buf + 1 : "none";
}
...
char *flag_room_name(int vector)
{
    ...
    if (vector & ROOM_NO_MAGIC)
        strcat(buf, " nomagic");
    if (vector & ROOM_NO_SUMMON)
        strcat(buf, " nosummon");
    if (vector & ROOM_NO_GATE)
        strcat(buf, " nogate");
    if (vector & ROOM_NO_CONSECRATE)
        strcat(buf, " noconsecrate");
    if (vector & ROOM_LOW_ONLY)
        strcat(buf, " lowonly");
    if (vector & ROOM_NO_MAGIC)
        strcat(buf, " nomagic");
    if (vector & ROOM_CONSECRATED)
        strcat(buf, " consecrated");
    if (vector & ROOM_NO_NEWBIE)
