	 ...
	 */
	{
		...
		for (;;)
		{
			...
			for (;;)
			{
				...
				else if (!str_cmp(word, "IMPROGS"))
				{
					load_improgs(fpArea);
				}
				else if (!str_cmp(word, "ROOMDATA"))
				{
					load_rooms_new(fpArea);
				}
				else if (!str_cmp(word, "MOBDATA"))
				{
					load_mobiles_new(fpArea);
				}
				else if (!str_cmp(word, "OBJDATA"))
				{
					load_objects_new(fpArea);
				}
				else
				{
...
int load_materials(FILE *fp)
{
	...
	// Comments
	while (strcmp(fread_word(fp), "MATS"))
	{
		fread_to_eol(fp);
	}
...
void load_old_obj(FILE *fp)
{
	...
	for (;;)
	{
		...
		{
			if (is_name("two", pObjIndex->name) || is_name("two-handed", pObjIndex->name) || is_name("spear", pObjIndex->name) || is_name("staff", pObjIndex->name) || is_name("polearm", pObjIndex->name) || is_name("halberd", pObjIndex->name) || is_name("spike", pObjIndex->name) || is_name("claymore", pObjIndex->name))
...
void load_resets(FILE *fp)
{
	...
	for (;;)
	{
		...
		case 'D':
			...
			if (pReset->arg2 < 0 || pReset->arg2 > (MAX_DIR - 1) || !pRoomIndex || !(pexit = pRoomIndex->exit[pReset->arg2]) || !IS_SET(pexit->rs_flags, EX_ISDOOR))
			{
...
void fix_exits(void)
{
	...
	for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
	{
			 ...
			 pRoomIndex = pRoomIndex->next)
		{
			for (door = 0; door <= 5; door++)
			{
				if ((pexit = pRoomIndex->exit[door]) != NULL && (to_room = pexit->u1.to_room) != NULL && (pexit_rev = to_room->exit[rev_dir[door]]) != NULL && pexit_rev->u1.to_room != pRoomIndex && (pRoomIndex->vnum < 1200 || pRoomIndex->vnum > 1299))
				{
...
void area_update(void)
{
	...
	for (pArea = area_first; pArea != NULL; pArea = pArea->next)
	{
		 ...
		 */
		if ((!pArea->empty && (pArea->nplayer == 0 || pArea->age >= 15) && !pArea->explore) || (pArea->age >= 31 && !pArea->explore) || (pArea->explore && pArea->nplayer == 0 && pArea->age >= 15))
		{
			ROOM_INDEX_DATA *pRoomIndex;
			...
			if (pRoomIndex != NULL && pArea == pRoomIndex->area)
				pArea->age = 15 - 2;
			else if ((pRoomIndex = get_room_index(9401)) != NULL && pRoomIndex->area == pArea)
				pArea->age = 15 - 2;
			else if ((pRoomIndex = get_room_index(23100)) != NULL && pRoomIndex->area == pArea)
				pArea->age = 15 - 2;
			else if ((pRoomIndex = get_room_index(26000)) != NULL && pRoomIndex->area == pArea)
				pArea->age = 15 - 2;
...
void reset_room(ROOM_INDEX_DATA *pRoom)
{
	...
	for (pReset = pRoom->reset_first; pReset != NULL; pReset = pReset->next)
	{
		...
		case 'P':
			...
			if (pRoom->area->nplayer > 0 || (LastObj = get_obj_type(pObjToIndex)) == NULL || (LastObj->in_room == NULL && !last) || (pObjIndex->count >= limit /* && number_range(0,4) != 0 */) || (pObjIndex->limcount >= pObjIndex->limtotal && pObjIndex->limtotal > 0) || (count = count_obj_list(pObjIndex, LastObj->contains)) > pReset->arg4)
			...
			while (count < pReset->arg4)
			{
				...
				/*if (pObjIndex->count >= limit)
					break;*/
				if (pObjIndex->count >= limit && pObjIndex->limcount >= pObjIndex->limtotal && pObjIndex->limtotal > 0)
					break;
			}
		...
		case 'E':
			...
			else /* ROM OLC else version */
			{
				...
				if (pObjIndex->limcount >= pObjIndex->limtotal && pObjIndex->limtotal > 0 && FALSE)
				{
					break;
				}
				else if (pObjIndex->count < limit || number_range(0, 4) == 0)

				{
					pObj = create_object(pObjIndex,
										 UMIN(number_fuzzy(level), LEVEL_HERO - 1));
					/* error message if it is too high */
					if (pObj->level > LastMob->level + 3 || (pObj->item_type == ITEM_WEAPON && pReset->command == 'E' && pObj->level < LastMob->level - 5 && pObj->level < 45))
						;
					/* fprintf(stderr,
...
void do_areset args((CHAR_DATA * ch, char *argument))
{
	...
	for (pArea = area_first; pArea != NULL; pArea = pArea->next)
	{
		if (((pArea->vnum == areachoice) && ((areachoice != 0) || (areachoice != -1))) || (!str_cmp(arg, pArea->file_name)))
		{
			reset_area(pArea);
			pArea->age = number_range(0, 3);
		...
		case 'E':
			if (!str_cmp(word, "End"))
			{
				...
				if (pObjIndex->item_type == ITEM_ARMOR)
				{
					...
					if (IS_SET(pObjIndex->wear_flags, ITEM_WEAR_STRAPPED))
					{
						recommend_ac_cap = 0.5;
					}
				...
				if (IS_SET(pObjIndex->wear_flags, ITEM_WEAR_FINGER))
				{
					recommend_weight = 0.15;
				}
				if (IS_SET(pObjIndex->wear_flags, ITEM_WEAR_NECK))
				{
					recommend_weight = 0.4;
				}
				if (IS_SET(pObjIndex->wear_flags, ITEM_WEAR_BODY))
				{
					recommend_weight = 1;
				...
				if (IS_SET(pObjIndex->wear_flags, ITEM_WEAR_LEGS))
				{
					recommend_weight = 0.85;
				}
				if (IS_SET(pObjIndex->wear_flags, ITEM_WEAR_FEET))
				{
					recommend_weight = 0.7;
				}
				if (IS_SET(pObjIndex->wear_flags, ITEM_WEAR_HANDS))
				{
					recommend_weight = 0.6;
				}
				if (IS_SET(pObjIndex->wear_flags, ITEM_WEAR_ARMS))
				{
					recommend_weight = 0.8;
				...
				if (pObjIndex->item_type == ITEM_WEAPON)
				{
					...
					case WEAPON_STAFF:
						...
						break;
					case WEAPON_HAND:
						recommend_weight = 0.3;
						break;
					}
					if (IS_SET(pObjIndex->value[4], WEAPON_TWO_HANDS))
						recommend_weight *= 1.5;
				}

				if (pObjIndex->item_type == ITEM_BOAT)
					recommend_weight *= 4;
