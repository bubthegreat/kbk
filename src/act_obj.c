...
bool can_loot(CHAR_DATA *ch, OBJ_DATA *obj)
{
    ...
    {
        sprintf(buf, "%s looting %s.", IS_NPC(ch) ? ((ch->master == NULL) ? "Unknown mob" : ch->master->name) : ch->name, obj->short_descr);
        log_string(buf);
        ch->pause += 5;
    }
...
void do_put(CHAR_DATA *ch, char *argument)
{
    ...
    else
    {
        ...
        for (obj = ch->carrying; obj != NULL; obj = obj_next)
        {
            ...
            if ((arg1[3] == '\0' || is_name(&arg1[4], obj->name)) && can_see_obj(ch, obj) && WEIGHT_MULT(obj) == 100 && obj->wear_loc == WEAR_NONE && obj != container && obj->pIndexData->limtotal <= 0 && can_drop_obj(ch, obj) && get_obj_weight(obj) + get_true_weight(container) <= (container->value[0] * 10) && get_obj_weight(obj) < (container->value[3] * 10) && (!isCabalItem(obj)))
            {
...
void do_skin(CHAR_DATA *ch, char *argument)
{
    ...
    corpse = NULL;
    if ((get_skill(ch, gsn_skin) == 0) || (ch->level < skill_table[gsn_skin].skill_level[ch->class]))
    {
    ...
    {
        if (find_corpse->wear_loc == WEAR_NONE && (can_see_obj(ch, find_corpse)) && (arg1[0] == '\0' || is_name(arg1, find_corpse->name)))
        {
            if (++count == number)
                corpse = find_corpse;
        }
...
void do_butcher(CHAR_DATA *ch, char *argument)
{
    ...
    corpse = NULL;
    if ((get_skill(ch, gsn_butcher) == 0) || (ch->level < skill_table[gsn_butcher].skill_level[ch->class]))
    {
...
void do_embalm(CHAR_DATA *ch, char *argument)
{
    ...
    if ((get_skill(ch, gsn_embalm) == 0) || ch->level < skill_table[gsn_embalm].skill_level[ch->class])
    {
...
void do_trophy(CHAR_DATA *ch, char *argument)
{
    ...
    if (((chance = get_skill(ch, gsn_trophy)) == 0) || ch->level < skill_table[gsn_trophy].skill_level[ch->class])
    {
        send_to_char("Huh?\n\r", ch);
        return;
    }
...
void do_demand(CHAR_DATA *ch, char *argument)
{
    ...
    if (chance == 0 || ch->level < skill_table[gsn_demand].skill_level[ch->class])
    {
...
void obj_brandish_horn_valere(CHAR_DATA *ch, OBJ_DATA *obj)
{
    ...
    for (bane = ch->in_room->people; bane != NULL; bane = bane->next_in_room)
    {
        if (!IS_NPC(bane))
            continue;
        if (bane->pIndexData->vnum == 13718 && (bane->fighting == ch || ch->fighting == bane))
            break;
    }
    ...
    for (i = 1; i < MAX_CABAL; i++)
    {
        ...
        for (obj = object_list; obj != NULL; obj = obj->next)
            if (obj->pIndexData->vnum == vnum)
                fprintf(fp, "%i %li\n", i,
                        (obj->carried_by && IS_NPC(obj->carried_by) && IS_CABAL_GUARD(obj->carried_by)) ? obj->carried_by->pIndexData->vnum : 0);
    }
    fprintf(fp, "-1");
    fclose(fp);
}
...
void do_steal(CHAR_DATA *ch, char *argument)
{
    ...
    { // Should this be can_drop_obj(victim, obj) ?
    ...
    {
        if (!IS_NPC(room_char) && (get_skill(room_char, skill_lookup("intercept")) > 1) && IS_AWAKE(room_char) && (!can_see(victim, room_char) || !IS_AWAKE(victim)) && is_affected(room_char, skill_lookup("intercept")) && (!is_safe(victim, room_char) && !is_safe(ch, room_char)) && (room_char != ch && room_char != victim))
        {
...
void wield_obj(CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace)
{
    ...
    if ((ch->size < SIZE_LARGE && IS_WEAPON_STAT(obj, WEAPON_TWO_HANDS)) || (obj->value[0] == WEAPON_STAFF) || (obj->value[0] == WEAPON_POLEARM) || (obj->value[0] == WEAPON_SPEAR))
    {
