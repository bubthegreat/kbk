...
strcat( buf, "(Veiled) ");
    ...
    && victim->position == POS_STANDING && ch->on == NULL && !is_affected(victim,gsn_cloak_form) &&
...
void show_char_to_char( CHAR_DATA *list, CHAR_DATA *ch )
{
    ...
    if (!IS_NPC(ch)
    && is_affected_room(ch->in_room, skill_lookup("globe of darkness"))
...
void do_exits( CHAR_DATA *ch, char *argument )
{
	...
	&& pexit->u1.to_room != NULL
	&& IS_SET(pexit->exit_info,EX_NONOBVIOUS)
	&& (IS_IMMORTAL(ch) || is_affected(ch,gsn_acute_vision)) )
	{
		found = TRUE;
		if ( fAuto)
			{
...
void do_score( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char("You are ", ch);
        if (GET_AC(ch,i) / AC_PER_ONE_PERCENT_DECREASE_DAMAGE <=  -2 )    sprintf(buf,"hopelessly vulnerable to %s.\n\r",temp);
        else if (GET_AC(ch,i) / AC_PER_ONE_PERCENT_DECREASE_DAMAGE <=  -1 )    sprintf(buf,"defenseless against %s.\n\r",temp);
        else if (GET_AC(ch,i) / AC_PER_ONE_PERCENT_DECREASE_DAMAGE <=  0 )    sprintf(buf,"barely protected from %s.\n\r",temp);
        else if (GET_AC(ch,i) / AC_PER_ONE_PERCENT_DECREASE_DAMAGE <=  1 )    sprintf(buf,"slightly armored against %s.\n\r",temp);
        else if (GET_AC(ch,i) / AC_PER_ONE_PERCENT_DECREASE_DAMAGE <=  3 )    sprintf(buf,"somewhat armored against %s.\n\r",temp);
        else if (GET_AC(ch,i) / AC_PER_ONE_PERCENT_DECREASE_DAMAGE <=  6 )     sprintf(buf,"armored against %s.\n\r",temp);
        else if (GET_AC(ch,i) / AC_PER_ONE_PERCENT_DECREASE_DAMAGE <=  10 )   sprintf(buf,"well-armored against %s.\n\r",temp);
        else if (GET_AC(ch,i) / AC_PER_ONE_PERCENT_DECREASE_DAMAGE <=  15 )   sprintf(buf,"very well-armored against %s.\n\r",temp);
        else if (GET_AC(ch,i) / AC_PER_ONE_PERCENT_DECREASE_DAMAGE <=  21 )  sprintf(buf,"heavily armored against %s.\n\r",temp);
        else if (GET_AC(ch,i) / AC_PER_ONE_PERCENT_DECREASE_DAMAGE <=  28 )  sprintf(buf,"superbly armored against %s.\n\r",temp);
        else if (GET_AC(ch,i) / AC_PER_ONE_PERCENT_DECREASE_DAMAGE <=  36 )  sprintf(buf,"supremely armored against %s.\n\r",temp);
        else if (GET_AC(ch,i) / AC_PER_ONE_PERCENT_DECREASE_DAMAGE <=  45 )  sprintf(buf,"almost invulnerable to %s.\n\r",temp);
        else sprintf(buf,"divinely armored against %s.\n\r",temp);
	send_to_char(buf,ch);
    }
...
if ( ch->level >= 15 )
	...
	if (saves_chance > 100) saves_chance = 100;
...
void do_help( CHAR_DATA *ch, char *argument )
{
	...
	if (pHelp->level < 61)
		continue;
        if ((pHelp->level == 61 && ch->cabal != CABAL_ANCIENT)
        || ( pHelp->level == 62 && ch->cabal != CABAL_ARCANA)
        || ( pHelp->level == 63 && ch->cabal != CABAL_BOUNTY)
        || ( pHelp->level == 64 && ch->cabal != CABAL_KNIGHT)
        || ( pHelp->level == 65 && ch->cabal != CABAL_OUTLAW)
        //|| ( pHelp->level == 66 && ch->cabal != CABAL_ENFORCER)
        || ( pHelp->level == 66 && ch->cabal != CABAL_RAGER) 
	|| ( pHelp->level == 67 && ch->cabal != CABAL_SYLVAN))
	    continue;
    	}
...
void do_whois (CHAR_DATA *ch, char *argument)
{
	...
	if (str_cmp(wch->name, wch->original_name) ? 
			IS_IMMORTAL(ch) ?
				!str_prefix(arg,wch->original_name) : 
			is_affected(wch,skill_lookup("cloak")) && ch->cabal == CABAL_ANCIENT ? 
				!str_prefix(arg,wch->original_name) : 
				0 :
...
void do_who( CHAR_DATA *ch, char *argument )
{
	...
	else
	{
        sprintf( buf, "[%2d %5s %s] %s%s%s%s%s%s%s%s%s%s%s%s%s%s\n\r",
			...
			isNewbie(wch) ? "(NEWBIE) " : "",
			IS_SET(wch->act, PLR_CRIMINAL) ? "(WANTED) " : "",
            		(ch->cabal == CABAL_ANCIENT) && (wch->original_name) ? 
					is_affected(wch,skill_lookup("cloak")) ? wch->original_name : wch->name : wch->name,
			IS_NPC(wch) ? "" : wch->pcdata->title,
			IS_NPC(wch) ? "" : (wch->pcdata->extitle) ? wch->pcdata->extitle : "");
		add_buf(output,buf);
	}
    ...
    for (rch = ch->in_room->people; rch != NULL; rch = rch->next_in_room)
    {
		...
		if ( (arg[0] != '\0') ) {
			if (!is_name(arg,rch->name)) {
				if (!rch->original_name) continue;
				else {
					if (IS_IMMORTAL(ch) && is_name(arg,rch->original_name)) goto found;

					if (is_affected(rch,skill_lookup("cloak")) && ch->cabal == CABAL_ANCIENT && is_name(arg,rch->original_name)) goto found;
