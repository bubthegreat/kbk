...
const struct olc_help_type help_table[] =
{
    ...
    {   "restrict",	restrict_flags,  "Restrict objects."		 },
...
/* ROM specific bits: */
    ...
    {	"extended",	extended_flags,	 "Extended Offense Flags."	},
    {	NULL,		NULL,		 NULL				 }
};
...
void show_spec_cmds( CHAR_DATA *ch )
{
    ...
    {
	sprintf( buf, "spec_%-19.18s", &spec_table[spec].name[5] );
	strcat( buf1, buf );
	if ( ++col % 4 == 0 )
	    strcat( buf1, "\n\r" );
    }
...
AEDIT( aedit_explore )
{
    ...
    if (pArea->explore == TRUE) {
	pArea->explore = FALSE;
    	send_to_char("This area is no longer an EXPLORE area.\n\r", ch);
    } else {
	pArea->explore = TRUE;
	send_to_char("This area is now an EXPLORE area.\n\r", ch);
    }
...
/* ARMOR for ROM */
        ...
        case ITEM_ARMOR:
	    sprintf( buf,
		"[{y(Reference) AC per 1%% redux: %d{x]\n\r"
		"[v0] Ac pierce       [%ld]\n\r"
		"[v1] Ac bash         [%ld]\n\r"
		"[v2] Ac slash        [%ld]\n\r"
		"[v3] Ac exotic       [%ld]\n\r",
		(int)AC_PER_ONE_PERCENT_DECREASE_DAMAGE,
		obj->value[0],
		obj->value[1],
		obj->value[2],
		obj->value[3] );
...
OEDIT( oedit_show )
{
	...
	bitmask_string( &pObj->extra_flags,  extra_flags ) );
    send_to_char( buf, ch );

	sprintf( buf, "Restricted:  [%s]\n\r",
    	flag_string( restrict_flags, pObj->restrict_flags ) );
	send_to_char( buf, ch );
...
OEDIT( oedit_copy )
{
    ...
    pObj->restrict_flags = pObj2->restrict_flags;

    pObj->material = pObj2->material;
    
    pObj->condition = pObj2->condition;

    pObj->limtotal = pObj2->limtotal;
...
OEDIT( oedit_addaffect )
{
    ...
    pAf             =   new_affect();
    pAf->location   =   value;
	if (value == APPLY_DAM_MOD || value == APPLY_ENHANCED_MOD || value == APPLY_ATTACK_MOD)
		send_to_char("{RREFRAIN FROM OVERUSING THIS STAT!!!{x\n\r",ch);
...
OEDIT( oedit_create )
{
    ...
    pObj->vnum			= value;
    pObj->area			= pArea;
	pObj->material		= find_material_by_name("none");
...

{
    ...
    if ( argument[0] != '\0' )
    {
        ...
        if ( ( value = flag_value( restrict_flags, argument ) ) != NO_FLAG )
        {
            TOGGLE_BIT(pObj->restrict_flags, value);
    ...
    send_to_char( "Syntax:  restrict [flag]\n\r"
                  "Type '? restrict' for a list of flags.\n\r", ch );
    return FALSE;
}
...
OEDIT( oedit_material )
{
	...
	if ((mat = find_material_by_name(argument)) == find_material_by_name("none")) {
		char buf[MSL],buf2[MSL];
		sprintf(buf,"Not a valid material. The following materials are valid:\n\r");
		mat = mat_list;
		 
		while (mat != NULL) {
			if (!str_cmp(mat->name,"none")) { mat = mat->next; continue; }
...
OEDIT( oedit_limit )
{
	...
	EDIT_OBJ( ch, pObj );
        ...
        pObj->limtotal = value;
	send_to_char( "Limit Object set.\n\r",ch );

	return TRUE;
    }

    send_to_char( "Syntax: limit [number]\n\r"
                  "Where number can range from 1 (unique) to 30 (regular).\n\r",ch );
    return FALSE;
}
...
OEDIT( oedit_cabal )
{
    ...
    if ( argument[0] != '\0' && ( cabal = cabal_lookup( argument ) ) != 0 )
    {
        EDIT_OBJ( ch, pObj );
    ...
    if (argument[0] != '\0' && !str_cmp(argument, "none"))
    {
        EDIT_OBJ( ch, pObj );
        pObj->cabal = 0;
        send_to_char("Object cabal status cleared.\n\r", ch);
        return TRUE;
    }

    send_to_char( "Syntax: cabal [cabal]\n\r", ch );
    return FALSE;
}
...
MEDIT( medit_show )
{
    ...
    sprintf( buf, "Dam Mod:             [%d%%]\n\r",
             pMob->dam_mod );
    send_to_char( buf, ch );

    sprintf( buf, "EnhancedDam Mod:     [%.0f]\n\r",
             pMob->enhancedDamMod );
    send_to_char( buf, ch );
	
	sprintf( buf, "Regen Rate:          [%d]\n\r",
			 pMob->regen_rate );
	send_to_char( buf, ch );
    ...
    sprintf( buf, "Armor:               [pierce: %d  bash: %d  slash: %d  magic: %d] [{y(Reference) AC per 1%% redux:%d{x]\n\r",
	...
	flag_string( extended_flags, pMob->extended_flags));
    send_to_char(buf, ch);
    ...
    sprintf( buf, "Start position:      [%s]\n\r",
	flag_string( position_flags, pMob->start_pos ) );
    send_to_char( buf, ch );

    sprintf( buf, "Default position:    [%s]\n\r",
...
/*    sprintf( buf, "Wealth:              [%5ld]\n\r",
    ...
    send_to_char( buf, ch );
*/
    sprintf( buf, "Quest Credits:       [%5ld]\n\r",
	pMob->quest_credit_reward );
    send_to_char( buf, ch );

    if ( pMob->spec_fun )
    {
	sprintf( buf, "Spec function:       [%s]\n\r",  spec_name( pMob->spec_fun ) );
	send_to_char( buf, ch );
    }
...
MEDIT( medit_copy )
{
    ...
    pMob->numAttacks = pMob2->numAttacks;
    ...
    pMob->enhancedDamMod = pMob2->enhancedDamMod;
	pMob->regen_rate = pMob2->regen_rate;
    ...
    pMob->quest_credit_reward = pMob2->quest_credit_reward;
...
MEDIT( medit_dammod )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ) )
    {
        send_to_char( "Syntax: dammod [number]\n\r", ch );
        return FALSE;
    }

    pMob->dam_mod = atoi( argument );

    send_to_char( "Damage mod set.\n\r", ch);
    return TRUE;
}
...
MEDIT( medit_qc )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ) )
    {
	send_to_char( "Syntax:  questcredits [number]\n\r", ch );
	return FALSE;
    }

    pMob->quest_credit_reward = atoi( argument );
    send_to_char( "Quest credits set.\n\r", ch);
    return TRUE;
}
...
MEDIT( medit_attackmod )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ) )
    {
	send_to_char( "Syntax:  attackmod [number]\n\r", ch );
	return FALSE;
    }

    if ( is_number( argument ) && atoi( argument ) > 20)
    {
	send_to_char( "Attackmod cannot be a value greater than 20. Lower it and try again.\n\r", ch);
	return FALSE;
    }
    if ( is_number( argument ) && atoi( argument ) < 0)
    {
	send_to_char( "Attackmod cannot be a negative value. Raise it and try again.\n\r", ch);
	return FALSE;
    }
    else
    {
	pMob->numAttacks = atoi( argument );
    	send_to_char( "Extra number of attacks modifier set.\n\r", ch);
    }
    return TRUE;
}

MEDIT( medit_regen_rate )
{
	MOB_INDEX_DATA *pMob;
    ...
    if ( is_number( argument ) && atoi( argument ) < 0)
    {
	send_to_char( "Regen rate cannot be a negative value. Raise it and try again.\n\r", ch);
	return FALSE;
    }

    else
    {
	pMob->regen_rate = atoi( argument );
    	send_to_char( "Regen rate modifier set.\n\r", ch);
    }
    return TRUE;
}

MEDIT( medit_enhancedDamMod )
{
    MOB_INDEX_DATA *pMob;
    ...
    if ( argument[0] == '\0' || !is_number( argument ) )
    {
	send_to_char( "Syntax:  enhancedDamMod [number]\n\r", ch );
	return FALSE;
    }

    else
    {
	pMob->enhancedDamMod = atoi( argument );
    	send_to_char( "Enhanced damage modifier set.\n\r", ch);
    }
    return TRUE;
}
...
MEDIT( medit_cabal )
{
	...
	EDIT_MOB( ch, pMob );
	pMob->cabal = 0;
	send_to_char("Mob cabal status cleared.\n\r", ch);
	return TRUE;
    }
...
MEDIT( medit_autohard )
{
    ...
    if (pMob->level < 1)
    {
	send_to_char("Set mobile level first.\n\r", ch);
	return FALSE;
    }

    ac_n = 88 - (pMob->level * 14.05) - ((pMob->level/10)^2);
    ac_x = 88 - (pMob->level * 8.02) - ((pMob->level/10)^2);
    ...
    send_to_char("Mob values set, check for accuracy.\n\r", ch);
    return TRUE;
}
...
REDIT( redit_cabal )
{
    ...
    if (argument[0] != '\0' && !str_cmp(argument, "none"))
    {
        EDIT_ROOM( ch, pRoom );
        pRoom->cabal = 0;
        send_to_char("Room cabal status cleared.\n\r", ch);
        return TRUE;
    }
...
MEDIT( medit_extended )
{
    ...
    if ( argument[0] != '\0' )
    {
        ...
        if ( ( value = flag_value( extended_flags, argument ) ) != NO_FLAG )
        {
            pMob->extended_flags ^= value;
            send_to_char( "Extended flags toggled.\n\r", ch );
            return TRUE;
        }
    }

    send_to_char( "Syntax: extended [flags]\n\r"
                  "Type '? extended' for a list of flags.\n\r", ch );
    return FALSE;
}
