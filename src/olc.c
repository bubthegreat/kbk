...
const struct olc_cmd_type medit_table[] =
	{
		...
		{"questcredits", medit_qc},
		{"attackmod", medit_attackmod},
		{"enhancedDamMod", medit_enhancedDamMod},
		{"regen", medit_regen_rate},
		...
		{"version", show_version},
		{"cabal", medit_cabal},
		{"automob", medit_automob}, /* TRACERAH */
		{"dammod", medit_dammod},
		{"extended", medit_extended},
		{"?", show_help},
...
void do_redit(CHAR_DATA *ch, char *argument)
{
	...
	if (!str_cmp(arg1, "reset"))
	{
		...
		{
			send_to_char("Insufficient security to modify quarter.\n\r", ch);
			return;
		}
	...
	else
	{
		...
		{
			send_to_char("Insufficient security to edit quarter or fourth nonexistent one.\n\r", ch);
			return;
		}
	}
	...
	if (!IS_BUILDER(ch, pRoom->area))
	{
		send_to_char("Insufficent security to edit area.\n\r", ch);
		return;
	}
...
void do_alist(CHAR_DATA *ch, char *argument)
{
	...
	if (!str_cmp(argument, "free"))
		return show_alist_free(ch, argument);
	...
	for (pArea = area_first; pArea; pArea = pArea->next)
	{
		sprintf(buf, "[%3ld] %-29.29s (%-5ld-%5ld) %-12.12s [%d] [%-10.10s]\n\r",
				pArea->vnum,
				pArea->name,
				pArea->min_vnum,
				pArea->max_vnum,
...
void do_progset(CHAR_DATA *ch, char *argument)
{
	OBJ_INDEX_DATA *pObj;
	MOB_INDEX_DATA *pMob;
	ROOM_INDEX_DATA *pRoom;
	...
	if (is_number(arg1) || !is_number(arg2) || is_number(arg3) || is_number(argument))
	{
		send_to_char("Syntax: progset <obj/mob> <vnum> <prog-type> <prog-name>\n"
					 "             prog-types: wear_prog,get_prog,fight_prog, etc.\n"
					 "             prog-names: none,get_prog_clan_item, etc.\n\r",
					 ch);
		return;
	}
	vnum = atoi(arg2);
	if (!str_cmp(arg1, "obj"))
	{
		...
		{
			send_to_char("Improgs: (Obj) That vnum does not exist.\n\r", ch);
			return;
		}
		if (pObj->iprogs == NULL)
			pObj->iprogs = alloc_perm(sizeof(IPROG_DATA));
		iprog_set(pObj, arg3, argument);

		sprintf(buf, "Obj prog %s for object %ld set at %s\n\r", arg3, vnum, argument);
		send_to_char(buf, ch);
	}
	else if (!str_cmp(arg1, "mob"))
	{
		...
		{
			send_to_char("Improgs: (Mob) That vnum does not exist.\n\r", ch);
			return;
		}
		if (pMob->mprogs == NULL)
			pMob->mprogs = alloc_perm(sizeof(MPROG_DATA));
		mprog_set(pMob, arg3, argument);

		sprintf(buf, "Mob prog %s for mobile %ld set at %s\n\r", arg3, vnum, argument);
		send_to_char(buf, ch);
	}
	else if (!str_cmp(arg1, "room"))
	{
		...
		{
			send_to_char("Progset: (Room) That vnum does not exist.\n\r", ch);
			return;
		}
		if (pRoom->rprogs == NULL)
			pRoom->rprogs = alloc_perm(sizeof(RPROG_DATA));
		rprog_set(pRoom, arg3, argument);

		sprintf(buf, "Room prog %s for room %ld set at %s\n\r", arg3, vnum, argument);
		send_to_char(buf, ch);
	}
