...
void violence_update(void)
{
    ...
    for (ch = char_list; ch != NULL; ch = ch_next)
    {
        ...
        if (IS_NPC(ch) && (!IS_NPC(ch->fighting)) && !IS_SET(ch->act, ACT_NO_TRACK) && !IS_AFFECTED(ch, AFF_CHARM) && !IS_SET(ch->act, ACT_IS_HEALER) && !IS_SET(ch->act, ACT_BANKER))
    ...
    for (rch = ch->in_room->people; rch != NULL; rch = rch_next)
    {
        ...
        if (IS_AWAKE(rch) && rch->fighting == NULL)
        {
            ...
            if (!IS_NPC(ch) || IS_AFFECTED(ch, AFF_CHARM))
            {
                ...
                if (((!IS_NPC(rch) && IS_SET(rch->act, PLR_AUTOASSIST)) || IS_AFFECTED(rch, AFF_CHARM)) && is_same_group(ch, rch) && !is_safe(rch, victim))
                    multi_hit(rch, victim, TYPE_UNDEFINED);
                continue;
            }
...
void multi_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt)
{
    ...
    if (get_eq_char(ch, WEAR_DUAL_WIELD) != NULL || count_hands(ch) == 0 || (get_eq_char(ch, WEAR_WIELD) != NULL && count_hands(ch) == 1))
        candual = TRUE;
...
void mob_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt)
{
    ...
    case (2):
        if (IS_SET(ch->off_flags, OFF_DISARM) || (get_weapon_sn(ch) != gsn_hand_to_hand && (IS_SET(ch->act, ACT_WARRIOR) || IS_SET(ch->act, ACT_THIEF))))
            do_disarm(ch, "");
        break;
...
void one_hit_new(CHAR_DATA *ch, CHAR_DATA *victim, int dt, bool specials, bool blockable, int addition, int multiplier, char *dnoun)
{
    ...
    if (!IS_NPC(ch) && ch->cabal == CABAL_RAGER && !cabal_down_new(ch, CABAL_RAGER, FALSE) && number_percent() <= 5 && specials == TRUE)
    {
    ...
    if (IS_NPC(ch) && ch->cabal == CABAL_RAGER && number_percent() < 15 && specials == TRUE && (IS_CABAL_OUTER_GUARD(ch) || IS_CABAL_GUARD(ch)))
    {
        send_to_char("You land an especially devastating blow!\n\r", ch);
        act("$n lands an especially devastating blow!", ch, 0, 0, TO_ROOM);
        dam *= 3;
    ...
    if (ch->cabal == CABAL_KNIGHT && get_skill(ch, skill_lookup("truestrike")) && number_percent() < ((get_skill(ch, skill_lookup("truestrike")) - 40) / 5) - 3 && !cabal_down_new(ch, CABAL_KNIGHT, FALSE) && specials == TRUE)
    {
...
bool damage_newer(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, int dam_type, bool show, bool blockable, int addition, int multiplier, char *dnoun, bool isPlural, bool showCharName, AFFECT_DATA *dealtByAffect, bool keep_invisible)
{
    ...
    if ((dt >= TYPE_HIT || dt == gsn_dual_wield) && ch != victim && blockable == HIT_BLOCKABLE && ch->in_room == victim->in_room && dam > 0)
    {
        ...
        if (check_shield_magnetism(ch, victim, dt))
            return FALSE;
    ...
    if (dam > 1 && ((IS_AFFECTED(victim, AFF_PROTECT_EVIL) && IS_EVIL(ch)) || (IS_AFFECTED(victim, AFF_PROTECT_GOOD) && IS_GOOD(ch))) && dam_type != DAM_TRUESTRIKE)
        dam -= dam / 4;
    ...
    if (!IS_NPC(victim) && victim->level >= LEVEL_IMMORTAL && victim->hit < 1 && !arena)
        victim->hit = 1;
    ...
    if (victim->position == POS_DEAD)
    {
        ...
        if (!IS_NPC(victim))
        {
            sprintf(log_buf, "%s killed by %s at %ld", victim->name,
                    (IS_NPC(ch) ? ch->short_descr : owner ? owner->name
                                                          : ch->name),
                    victim->in_room->vnum);
            log_string(log_buf);
        }
    ...
    */
    ...
    {
        if ((IS_SET(victim->act, ACT_WIMPY) && number_bits(2) == 0 && victim->hit < victim->max_hit / 5) || (IS_AFFECTED(victim, AFF_CHARM) && victim->master != NULL && victim->master->in_room != victim->in_room && !is_centurion(victim) && dt != gsn_parting_blow && dt != gsn_cutoff))
            do_flee(victim, "");
    }

    if (!IS_NPC(victim) && victim->hit > 0 && victim->hit <= victim->wimpy && victim->wait < PULSE_VIOLENCE / 2 && dt != gsn_parting_blow)
        do_flee(victim, "");
...
bool is_safe(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    if (!IS_NPC(victim) && victim->desc == NULL && !IS_NPC(ch) && victim->fighting != ch && ch->fighting != victim)
    {
    ...
    if (IS_NPC(victim))
    {
        ...
        {
            if (IS_AFFECTED(ch, AFF_CHARM) && IS_AFFECTED(victim, AFF_CHARM) && !can_pk(ch->master, victim->master))
                return TRUE;
        }
    }
    /* killing players */
    else
    {
        ...
        if (IS_NPC(ch))
        {
            ...
            if (IS_AFFECTED(ch, AFF_CHARM) && ch->master != NULL && ch->master->fighting != victim && !is_centurion(ch))
            {
...
bool is_safe_spell(CHAR_DATA *ch, CHAR_DATA *victim, bool area)
{
    ...
    if (IS_NPC(victim))
    {
        ...
        if (IS_SET(victim->act, ACT_TRAIN) || IS_SET(victim->act, ACT_PRACTICE) || IS_SET(victim->act, ACT_IS_HEALER) || IS_SET(victim->act, ACT_IS_CHANGER))
            return TRUE;
...
void check_killer(CHAR_DATA *ch, CHAR_DATA *victim)
{
     ...
     */
    if (IS_NPC(ch) || ch == victim || ch->level >= LEVEL_IMMORTAL || !is_cabal(ch) || IS_SET(ch->act, PLR_KILLER) || ch->fighting == victim)
        return;
...
float hitroll_multiplier(CHAR_DATA *ch, CHAR_DATA *victim, float max_multiplier)
{
    ...
    {
        hit_ratio = ch_hitroll == 0 ? ch_hitroll = 1 : ch_hitroll > 0 ? victim_hitroll / ch_hitroll
...
bool check_dodge(CHAR_DATA *attacker, CHAR_DATA *defender, int dt)
{
    ...
    // For simplicity...
    if (defender_hitroll <= 0)
        defender_hitroll = 1;
    if (attacker_hitroll <= 0)
        attacker_hitroll = 1;
...
void make_corpse(CHAR_DATA *killer, CHAR_DATA *ch)
{
    ...
    if (IS_NPC(ch))
    {
        ...
        if (ch->level < 30)
            gold = 10 * (ch->level) + number_range(1, 10);
        else if (ch->level <= 53)
        {
            gold = 52.1 * (ch->level - 30) + number_range(275, 300);
            gold -= pMobIndex->killed * 10;
        }
        else if (ch->level <= 60)
        {
            gold = 214.2 * (ch->level - 53) + number_range(1300, 1500);
            gold -= pMobIndex->killed * 30;
        }
        else if (ch->level <= 70)
        {
            gold = 500 * (ch->level - 60) + number_range(2700, 3000);
            gold -= pMobIndex->killed * 100;
        }

        // now we're getting to possibly high-level group kills, so the gold scales much faster to compensate
        else if (ch->level <= 75)
        {
            gold = 1200 * (ch->level - 70) + number_range(7500, 8000);
            gold -= pMobIndex->killed * 250;
        }
        else if (ch->level <= 80)
        {
            gold = 16000 * (ch->level - 75) + number_range(13300, 14000);
            gold -= pMobIndex->killed * 450;
        }
        else
        {
            gold = 20000 * (ch->level - 80) + number_range(90000, 94000);
            gold -= pMobIndex->killed * 1000;
        }
    ...
    for (obj = ch->carrying; obj != NULL; obj = obj_next)
    {
        ...
        if (obj->pIndexData->limcount > obj->pIndexData->limtotal && obj->pIndexData->limtotal > 0 && !IS_NPC(ch) && (number_percent() < (abs(1. - (obj->pIndexData->limtotal / obj->pIndexData->limcount)) * 100.)))
        {
...
void group_gain(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    for (gch = ch->in_room->people; gch != NULL; gch = gch->next_in_room)
    {
        ...
        for (obj = ch->carrying; obj != NULL; obj = obj_next)
        {
            ...
            if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_GOOD) && IS_GOOD(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_NEUTRAL) && IS_NEUTRAL(ch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_NEUT) && (ch->pcdata->ethos == 0)) || (IS_OBJ_STAT(obj, ITEM_ANTI_CHAOTIC) && (ch->pcdata->ethos < 0)) || (IS_WEAPON_STAT(obj, WEAPON_UNHOLY) && (owner)))
            {
...
void dam_message(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, bool immune, char *dnoun, bool isPlural, bool showCharName)
{
    ...
    if (is_affected(victim, skill_lookup("mana shield")) && victim->mana > dam)
    {
        vs = "{xis absorbed by the mana shield around";
        vp = "{xis absorbed by the mana shield around";
    }
    else
    {
        if (is_affected(victim, skill_lookup("mana shield")) && victim->mana <= dam)
...
void do_berserk(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_berserk)) == 0 || (IS_NPC(ch) && !IS_SET(ch->off_flags, OFF_BERSERK)) || (!IS_NPC(ch) && ch->level < skill_table[gsn_berserk].skill_level[ch->class]))
    {
...
void do_bash(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_bash)) == 0 || (IS_NPC(ch) && !IS_SET(ch->off_flags, OFF_BASH)) || (!IS_NPC(ch) && ch->level < skill_table[gsn_bash].skill_level[ch->class]))
    {
...
void do_dirt(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_dirt)) == 0 || (IS_NPC(ch) && !IS_SET(ch->off_flags, OFF_KICK_DIRT) && ch->pIndexData->vnum != MOB_VNUM_ENFORCER) || (!IS_NPC(ch) && ch->level < skill_table[gsn_dirt].skill_level[ch->class]))
    {
...
void do_trip(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_trip)) == 0 || (IS_NPC(ch) && !IS_SET(ch->off_flags, OFF_TRIP) && ch->pIndexData->vnum != MOB_VNUM_ENFORCER) || (!IS_NPC(ch) && ch->level < skill_table[gsn_trip].skill_level[ch->class]))
    {
    ...
    if (IS_AFFECTED(victim, AFF_FLYING) && !is_affected(victim, gsn_earthbind))
    {
...
void do_murder(CHAR_DATA *ch, char *argument)
{
    ...
    if ((IS_AFFECTED(ch, AFF_CHARM) || (IS_NPC(ch) && IS_SET(ch->act, ACT_PET))) && !is_centurion(ch))
        return;
...
void do_backstab(CHAR_DATA *ch, char *argument)
{
    ...
    else
    {
        ...
        else if ((ch->drain_level + ch->level) < 50)
            dam *= number_range(10, 25); /* 4,6 */
        else
            dam *= number_range(15, 30); /* 4,7 */
...
void do_circle_stab(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_circle)) == 0 || IS_NPC(ch) || (!IS_NPC(ch) && ch->level < skill_table[gsn_circle].skill_level[ch->class]))
    {
...
void do_lunge(CHAR_DATA *ch, char *argument)
{
    ...
    dam = 0;
    one_argument(argument, arg);
    if ((chance = get_skill(ch, gsn_lunge)) == 0 || (ch->level < skill_table[gsn_lunge].skill_level[ch->class]))
    {
    ...
    wield = get_eq_char(ch, WEAR_WIELD);
    if ((wield == NULL) || ((wield->value[0] != WEAPON_SWORD) && (wield->value[0] != WEAPON_SPEAR) && (wield->value[0] != WEAPON_POLEARM)))
    {
...
void do_herb(CHAR_DATA *ch, char *argument)
{
    ...
    if ((get_skill(ch, gsn_herb) == 0) || (ch->level < skill_table[gsn_herb].skill_level[ch->class]))
    {
        send_to_char("Huh?\n\r", ch);
        return;
    }
...
void do_cleave(CHAR_DATA *ch, char *argument)
{
    ...
    if (((chance = get_skill(ch, gsn_cleave)) == 0) || (ch->level < skill_table[gsn_cleave].skill_level[ch->class]))
    {
...
void do_deathstrike(CHAR_DATA *ch, char *argument)
{
    ...
    if ((get_skill(ch, gsn_deathstrike) == 0) || ch->level < skill_table[gsn_deathstrike].skill_level[ch->class])
    {
        send_to_char("Huh?\n\r", ch);
        return;
    }
...
void do_crush(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_crush)) == 0 || ch->level < skill_table[gsn_crush].skill_level[ch->class])
        if (!IS_NPC(ch))
        {
...
void do_riot(CHAR_DATA *ch, char *argument)
{
    ...
    if ((get_skill(ch, gsn_riot) == 0) || ch->level < skill_table[gsn_riot].skill_level[ch->class])
    {
        send_to_char("Huh?\n\r", ch);
        return;
    }
...
void do_tail(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_tail)) == 0 || (IS_NPC(ch) && !IS_SET(ch->off_flags, OFF_TAIL)) || (!IS_NPC(ch) && ch->level < skill_table[gsn_tail].skill_level[ch->class]))
    {
...
void do_spellbane(CHAR_DATA *ch, char *argument)
{
    ...
    if (get_skill(ch, gsn_spellbane) < 10 || ch->level < skill_table[gsn_spellbane].skill_level[ch->class])
    {
        send_to_char("Huh?\n\r", ch);
        return;
    }
...
void do_throw(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_throw)) == 0 || ch->level < skill_table[gsn_throw].skill_level[ch->class])
    {
...
void do_palm(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_palm)) == 0 || ch->level < skill_table[gsn_palm].skill_level[ch->class])
    {
...
void do_nerve(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_nerve)) == 0 || ch->level < skill_table[gsn_nerve].skill_level[ch->class])
    {
...
void do_blindness_dust(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_blindness_dust)) == 0 || ch->level < skill_table[gsn_blindness_dust].skill_level[ch->class])
    {
    ...
    for (vch = ch->in_room->people; vch != NULL; vch = vch_next)
    {
        ...
        if (is_same_group(ch, vch))
            continue;
        if (!IS_AFFECTED(vch, AFF_BLIND) && !saves_spell(ch->drain_level + ch->level, vch, DAM_OTHER))
        {
...
void do_poison_dust(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_poison_dust)) == 0 || ch->level < skill_table[gsn_poison_dust].skill_level[ch->class])
    {
    ...
    for (vch = ch->in_room->people; vch != NULL; vch = vch_next)
    {
        ...
        if (is_same_group(ch, vch))
            continue;
        if (!IS_AFFECTED(vch, AFF_POISON) && !saves_spell(ch->drain_level + ch->level, vch, DAM_OTHER))
        {
...
bool check_roll(CHAR_DATA *ch, CHAR_DATA *victim, int dt)
{
    ...
    if ((chance = get_skill(victim, gsn_roll)) == 0 || victim->level < skill_table[gsn_roll].skill_level[victim->class])
        return FALSE;
...
void do_warcry(CHAR_DATA *ch, char *argument)
{
    AFFECT_DATA af;
    int chance;
    if ((chance = get_skill(ch, gsn_warcry)) == 0 || ch->level < skill_table[gsn_warcry].skill_level[ch->class])
    {
...
void do_enlist(CHAR_DATA *ch, char *argument)
{
    ...
    if (chance <= 10 || ch->level < skill_table[gsn_enlist].skill_level[ch->class])
    {
...
void do_find_water(CHAR_DATA *ch, char *argument)
{
    ...
    if (chance > 95)
        chance = 95;
    if (chance == 0 || ch->level < skill_table[gsn_find_water].skill_level[ch->class])
    {
...
void do_track(CHAR_DATA *ch, char *argument)
{
    ...
    if (chance == 0 || ch->level < skill_table[gsn_track].skill_level[ch->class])
    {
    ...
    if (ch->in_room->sector_type == SECT_INSIDE)
        ...
        chance -= 35;
    else if (ch->in_room->sector_type >= SECT_WATER_SWIM && ch->in_room->sector_type < SECT_DESERT)
        chance = 0;
    ...
    if (number_percent() > chance || victim == NULL || !IS_NPC(victim) || dir == 0)
    {
...
void do_shield_cleave(CHAR_DATA *ch, char *argument)
{
    ...
    if (chance == 0 || ch->level < skill_table[gsn_shield_cleave].skill_level[ch->class])
    {
    ...
    if (weapon == NULL || (weapon->value[0] != WEAPON_SWORD && weapon->value[0] != WEAPON_AXE))
    {
    ...
    if (is_affected(victim, gsn_shield_of_redemption))
    {
        ...
        act("$N's divine shield shatters, consuming $n in a torrent of holy energy!", ch, 0, victim, TO_NOTVICT);
...
void do_breath_fire(CHAR_DATA *ch, char *argument)
{
    ...
    if (chance == 0 || ch->level < skill_table[gsn_breath_fire].skill_level[ch->class])
    {
...
void do_rear_kick(CHAR_DATA *ch, char *argument)
{
    ...
    */
    ...
    if (!IS_NPC(ch) && ch->level < skill_table[gsn_rear_kick].skill_level[ch->class])
    {
...
void do_forage(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_forage)) == 0 || (ch->level < skill_table[gsn_forage].skill_level[ch->class]))
    {
...
void do_defend(CHAR_DATA *ch, char *argument)
{
    ...
    one_argument(argument, arg);
    if ((get_skill(ch, gsn_defend) == 0) || (ch->level < skill_table[gsn_defend].skill_level[ch->class]))
    {
...
void do_flee(CHAR_DATA *ch, char *argument)
{
    ...
    for (attempt = 0; attempt < 6; attempt++)
    {
        ...
        if (pexit->u1.to_room == NULL || (IS_SET(pexit->exit_info, EX_CLOSED) && (!IS_AFFECTED(ch, AFF_PASS_DOOR))) || number_range(0, ch->daze) != 0 || (IS_NPC(ch) && IS_SET(pexit->u1.to_room->room_flags, ROOM_NO_MOB)))
            continue;
...
void do_pugil(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_pugil)) == 0 || IS_NPC(ch) || (!IS_NPC(ch) && ch->level < skill_table[gsn_pugil].skill_level[ch->class]))
    {
...
void do_protection_heat_cold(CHAR_DATA *ch, char *argument)
{
    ...
    if (chance == 0 || ch->level < skill_table[gsn_protection_heat_cold].skill_level[ch->class])
    {
...
void do_undead_drain(CHAR_DATA *ch, char *argument)
{
    ...
    {
        if (vch->fighting == ch && number_percent() > 70)
            break;
    }
...
void do_quiet_movement(CHAR_DATA *ch, char *argument)
{
    ...
    if (chance == 0 || ch->level < skill_table[gsn_quiet_movement].skill_level[ch->class])
    {
...
void do_spike(CHAR_DATA *ch, char *argument)
{
    ...
    if ((get_skill(ch, gsn_spike) == 0) || ch->level < skill_table[gsn_spike].skill_level[ch->class])
    {
        send_to_char("Huh?\n\r", ch);
        return;
    }
        ...
        for (paf = ch->affected; paf != NULL; paf = paf_next)
        {
            ...
            if (paf->type == sn)
            {
                ...
                case APPLY_HIT:
                    ...
                    break;
                case APPLY_MANA:
                    manamod = paf->modifier;
                    affect_remove(ch, paf);
                    break;
                case APPLY_HITROLL:
                    hrollmod = paf->modifier;
                    affect_remove(ch, paf);
                    break;
                case APPLY_DAMROLL:
                    drollmod = paf->modifier;
                    affect_remove(ch, paf);
                    break;
                }
            }
...
void do_ambush(CHAR_DATA *ch, char *argument)
{
    ...
    if (victim->fighting != NULL && (chance < 3 || ch->level < skill_table[gsn_moving_ambush].skill_level[ch->class]))
    {
        send_to_char("They are moving around too much to ambush.\n\r", ch);
        return;
    }

    if (victim->fighting != NULL && number_percent() >= chance)
    {
    ...
    else if (!str_prefix(argument, "whirl"))
    {
        ...
        for (target = ch->in_room->people; target != NULL; target = targetLooper)
        {
            ...
            if (target->fighting == ch)
            {
                ...
                if (whirl == 1)
                    sprintf(message, "$n slashes you across the chest!");
                else if (whirl == 2)
                    sprintf(message, "$n's slash catches you in the arm!");
                else if (whirl == 3)
                    sprintf(message, "$n's slash connects with your stomach!");
                else if (whirl == 4)
                    sprintf(message, "$n's slash cleaves into your leg!");
...
void do_lash(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_AFFECTED(victim, AFF_FLYING))
        chance -= dice(2, 5);
    if (!IS_NPC(ch) && (ch->class == CLASS_CHANNELER || ch->class == CLASS_NECROMANCER || ch->class == CLASS_ELEMENTALIST))
        chance -= 10;
...
void check_critical_strike(CHAR_DATA *ch, CHAR_DATA *victim, int dt)
{
    ...
    {
        if (is_wielded(ch, WEAPON_SWORD, WIELD_PRIMARY) || is_wielded(ch, WEAPON_AXE, WIELD_PRIMARY) || is_wielded(ch, WEAPON_DAGGER, WIELD_PRIMARY) || is_wielded(ch, WEAPON_SPEAR, WIELD_PRIMARY) || is_wielded(ch, WEAPON_POLEARM, WIELD_PRIMARY) || is_wielded(ch, WEAPON_EXOTIC, WIELD_PRIMARY))
        {
        ...
        else if (is_wielded(ch, WEAPON_FLAIL, WIELD_PRIMARY) || is_wielded(ch, WEAPON_MACE, WIELD_PRIMARY) || is_wielded(ch, WEAPON_STAFF, WIELD_PRIMARY) || is_wielded(ch, WEAPON_WHIP, WIELD_PRIMARY))
        {
            act("You deliver a critical strike!", ch, 0, 0, TO_CHAR);
            act("$n delivers a critical strike!", ch, 0, 0, TO_ROOM);
            one_hit_new(ch, victim, dt, HIT_NOSPECIALS, HIT_UNBLOCKABLE, HIT_NOADD, 115, "critical strike");
