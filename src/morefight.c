...
void do_bearcharge(CHAR_DATA *ch, char *argument)
{
	...
	if ((chance = get_skill(ch, gsn_bearcharge)) == 0 || (IS_NPC(ch) && !IS_SET(ch->off_flags, OFF_BASH) && ch->pIndexData->vnum != MOB_VNUM_ENFORCER) || (!IS_NPC(ch) && ch->level < skill_table[gsn_bearcharge].skill_level[ch->class]))
	{
	...
	if (number_percent() < chance)
	{
		...
		if (!IS_NPC(ch) && (ch->class == CLASS_WARRIOR) && (number_percent() < ch->level))
			ch->pcdata->sp++;
	}
	else
	{
...
void do_whirl(CHAR_DATA *ch, char *argument)
{
	...
	int exedam;
	one_argument(argument, arg);
	if ((chance = get_skill(ch, gsn_whirl)) == 0 || (ch->level < skill_table[gsn_whirl].skill_level[ch->class] && !IS_NPC(ch)))
	{
		send_to_char("Huh??\n\r", ch);
		return;
	}
...
void do_findwood(CHAR_DATA *ch, char *argument)
{
	...
	if ((chance = get_skill(ch, gsn_fwood)) == 0 || (ch->level < skill_table[gsn_fwood].skill_level[ch->class]))
	{
		send_to_char("Huh?\n\r", ch);
		return;
	}
...
void do_carve(CHAR_DATA *ch, char *argument)
{
	...
	if ((chance = get_skill(ch, gsn_carve)) == 0 || (ch->level < skill_table[gsn_carve].skill_level[ch->class]))
	{
		send_to_char("Huh?\n\r", ch);
		return;
	}
...
void do_dash(CHAR_DATA *ch, char *argument)
{
	...
	if (!str_prefix(word, "dash") || !str_prefix(word, "east") || !str_prefix(word, "north") || !str_prefix(word, "west") || !str_prefix(word, "south") || !str_prefix(word, "up") || !str_prefix(word, "down"))
