...
void do_pktrack(CHAR_DATA *ch, char *argument) {
	...
	if		(!str_cmp(arg1,"wins"))		mysql_safe_query("SELECT * FROM pklogs WHERE killer RLIKE '%s'", argument);
	else if	(!str_cmp(arg1,"losses"))	mysql_safe_query("SELECT * FROM pklogs WHERE dead RLIKE '%s'", argument);
	else if	(!str_cmp(arg1,"all"))		mysql_safe_query("SELECT * FROM pklogs WHERE killer RLIKE '%s' OR dead RLIKE '%s'", argument, argument);
	else if	(!str_cmp(arg1,"date"))		mysql_safe_query("SELECT * FROM pklogs WHERE time RLIKE '%s'", argument);
	else if	(!str_cmp(arg1,"location"))	mysql_safe_query("SELECT * FROM pklogs WHERE room RLIKE '%s'", argument);
