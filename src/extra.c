...
void do_finger(CHAR_DATA *ch, char *argument)
{
	...
	else
	{
		...
		sprintf(buf2, "Cabal: %s%s%s, Align: %s, Ethos: %s\n\r",
				...
				align < 0 ? "evil" : align == 0 ? "neutral"
								 : align > 0	? "good"
												: "(none)",
				ethos < 0 ? "chaotic" : ethos == 0 ? "neutral"
									: ethos > 0	   ? "lawful"
												   : "(none)");
		send_to_char(buf2, ch);
		...
		for (paf = obj->pIndexData->affected; paf != NULL; paf = paf->next)
		{
			...
			if (paf->bitvector)
			{
				...
				case TO_AFFECTS:
					...
					break;
				case TO_OBJECT:
					sprintf(buf, "Adds %s object flag.", extra_bit_name(paf->bitvector));
					break;
				case TO_IMMUNE:
		...
		if (paf->location != APPLY_NONE && paf->modifier != 0)
		{
			...
			if (paf->bitvector)
			{
				...
				case TO_OBJECT:
					...
					break;
				case TO_WEAPON:
					sprintf(buf, "Adds %s weapon flags.", weapon_bit_name(paf->bitvector));
					break;
				case TO_IMMUNE:
					sprintf(buf, "Adds immunity to %s.", imm_bit_name(paf->bitvector));
					break;
...
void do_prep(CHAR_DATA *ch)
{
	...
	if ((chance = get_skill(ch, gsn_asscounter)) == 0 || (!IS_NPC(ch) && ch->level < skill_table[gsn_asscounter].skill_level[ch->class]))
	{
...
void do_siton(CHAR_DATA *ch, char *argument)
{
	...
	if ((get_skill(ch, gsn_siton) == 0) || ch->level < skill_table[gsn_siton].skill_level[ch->class])
	{
		if (!IS_NPC(ch))
		{
...
void do_knife(CHAR_DATA *ch, char *argument)
{
	...
	if ((get_skill(ch, gsn_knife) == 0) || ch->level < skill_table[gsn_knife].skill_level[ch->class])
	{
		if (!IS_NPC(ch))
		{
...
void do_praclist(CHAR_DATA *ch, char *argument)
{
	...
	for (sn = 0; sn < MAX_SKILL; sn++)
	{
		...
		if (skill_table[sn].skill_level[victim->class] > 52)
			continue;
		if (victim->level < skill_table[sn].skill_level[victim->class] || victim->pcdata->learned[sn] < 1 /* skill is not known */)
			continue;
...
void do_ccb(CHAR_DATA *ch, char *argument)
{
	...
	for (d = descriptor_list; d != NULL; d = d->next)
	{
		...
		{
			if (((d->character != ch) && (!IS_SET(d->character->comm, COMM_NOCABAL)) && (d->character->cabal) && (d->character->cabal == cabal)) || ((IS_SET(d->character->comm, COMM_ALL_CABALS)) && (d->character != ch)))
			{
...
void do_shieldbash_templar(CHAR_DATA *ch, char *argument)
{
	...
	if (!(IS_NPC(victim)) && !(IS_NPC(ch)) && victim->position > POS_STUNNED && !FightingCheck)
	{
...
void do_powers(CHAR_DATA *ch, char *argument)
{
	...
	for (gns = 0; gns < MAX_SKILL; gns++)
	{
		...
		if ((level = skill_table[sn].skill_level[ch->class]) < LEVEL_HERO + 1 && level >= min_lev && level <= max_lev && ch->pcdata->learned[sn] > 0)
		{
			found = TRUE;
...
bool check_chargeset(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	if ((chance = get_skill(victim, gsn_chargeset)) == 0 || victim->level < skill_table[gsn_chargeset].skill_level[ch->class])
		return FALSE;
