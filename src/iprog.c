	...
	if (number_percent() < 5)
	{
		...
		for (i = 0; i < numhits; i++)
		{
			if (ch->in_room != victim->in_room)
				break;
			damage_new(ch, victim, number_range(40, 70), TYPE_UNDEFINED, DAM_BASH, TRUE, HIT_UNBLOCKABLE, HIT_NOADD, HIT_NOMULT, "brutal assault");
			continue;
		}
	}
	return;
...
void invoke_prog_black_sapphire(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	{
		if (d->connected == CON_PLAYING && d->character->in_room != NULL && ch->in_room != NULL && d->character->in_room->area == ch->in_room->area && d->character != ch)
		{
...
void invoke_prog_rainbow_moonstone(OBJ_DATA *obj, CHAR_DATA *ch, char *argument)
{
	...
	if (victim->pIndexData->vnum == MOB_VNUM_FIRE_ELEMENTAL)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_WATER_ELEMENTAL)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_AIR_ELEMENTAL)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_EARTH_ELEMENTAL)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_ZOMBIE)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_BEAST)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_UNICORN)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_G_GOLEM)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_L_GOLEM)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_WOLF)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_BEAR)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_LION)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_FALCON)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_SPITTING_SNAKE)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_ARMOR)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_WEAPON)
		kill = TRUE;
	else if (victim->pIndexData->vnum == MOB_VNUM_KEY)
		kill = TRUE;
...
void fight_prog_training_pendant(OBJ_DATA *obj, CHAR_DATA *ch)
{
	...
	if (ch->pcdata->newbie != TRUE)
		return;
	if (ch->in_room->area == NULL || str_cmp(ch->in_room->area->name, "Training Grounds"))
		return;
