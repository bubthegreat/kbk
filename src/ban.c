...
void ban_site(CHAR_DATA *ch, char *argument, bool fPerm)
{
    ...
    if (arg1[0] == '\0')
    {
        ...
        add_buf(buffer, "Site                       Banned By    Level Type     Status\n\r");
        add_buf(buffer, "-------------------------------------------------------------\n\r");
        for (pban = ban_list; pban != NULL; pban = pban->next)
        {
            sprintf(buf2, "%s%s%s",
            ...
            sprintf(buf, "%-30s %-12s %-5d %-7s  %s\n\r",
                    buf2, pban->banner, pban->level,
                    IS_SET(pban->ban_flags, BAN_NEWBIES) ? "newbies" : IS_SET(pban->ban_flags, BAN_PERMIT) ? "permit"
