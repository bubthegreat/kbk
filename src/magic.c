...
bool eye_of_beholder_nullify(CHAR_DATA *ch)
{
    ...
    if (eye == NULL || eye->pIndexData->vnum != 14003)
        return FALSE;
...
void say_spell(CHAR_DATA *ch, int sn)
{
    ...
    if ((sn == skill_lookup("power word kill")) || (sn == skill_lookup("power word stun")))
        return;
...
void do_laying_hands(CHAR_DATA *ch, char *argument)
{
    ...
    if ((get_skill(ch, gsn_laying_hands) == 0) || (ch->level < skill_table[gsn_laying_hands].skill_level[ch->class]))
    {
...
void do_cast(CHAR_DATA *ch, char *argument)
{
    ...
    if (is_affected(ch, gsn_tigerclaw))
    {
        ...
        { // It goes through, and less damage
    ...
    if ((sn = find_spell(ch, arg1)) < 1 || skill_table[sn].spell_fun == spell_null || (ch->level < skill_table[sn].skill_level[ch->class] && !IS_IMMORTAL(ch)) || (IS_NPC(ch)) || ch->pcdata->learned[sn] == 0)
    {
    ...
    else
    {
        ...
        if (skill_table[sn].target == TAR_CHAR_DEFENSIVE || skill_table[sn].target == TAR_CHAR_OFFENSIVE)
            reflected = spellbaned(ch, victim, sn);

        if (skill_table[sn].target == TAR_CHAR_OFFENSIVE)
        {
            if (!IS_NPC(ch) && !IS_NPC(victim) && (ch->fighting == NULL || victim->fighting == NULL) && str_cmp(skill_table[sn].name, "incinerate") != 1)
            {
...
bool check_absorb(CHAR_DATA *ch, CHAR_DATA *victim, int dt)
{
    ...
    if (dt >= 0 && dt < MAX_SKILL)
    {
        ...
        if (skill_table[dt].ctype == CMD_COMMUNE || (skill_table[dt].ctype == CMD_BOTH && class_table[ch->class].ctype == CLASS_COMMUNER))
            return FALSE;
...
void spell_charm_person(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if (IS_AFFECTED(victim, AFF_CHARM) || IS_AFFECTED(ch, AFF_CHARM) || level < victim->level || IS_CABAL_GUARD(victim) || IS_SET(victim->imm_flags, IMM_CHARM) || (IS_NPC(victim) && victim->spec_fun != NULL) || (IS_NPC(victim) && IS_SET(victim->act, ACT_AGGRESSIVE)))
    {
...
void spell_electric_storm(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    AFFECT_DATA af;
    ...
    {
        send_to_char("Your unable to call upon the storm again so soon.\n\r", ch);
        return;
    }
...
void spell_cure_disease(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if (!str_cmp(class_table[ch->class].name, "monk") && victim != ch)
    {
...
void spell_curse(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    /* character curses */
    victim = (CHAR_DATA *)vo;
    if ((obj = get_eq_char(victim, WEAR_BODY)) != NULL && obj->pIndexData->vnum == 14005)
        level -= 20;
...
void spell_heat_metal(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if (!saves_spell(level + 2, victim, DAM_FIRE) && !IS_SET(victim->imm_flags, IMM_FIRE))
    {
             ...
             obj_lose = obj_next)
        {
            ...
            if (number_range(1, 2 * level) > obj_lose->level && !saves_spell(level, victim, DAM_FIRE) && IS_OBJ_STAT(obj_lose, ITEM_METAL))
            {
                ...
                case ITEM_ARMOR:
                    ...
                    {
                        if (can_drop_obj(victim, obj_lose) && (obj_lose->weight / 10) < number_range(1, 2 * get_curr_stat(victim, STAT_DEX)) && remove_obj(victim, obj_lose->wear_loc, TRUE))
                        {
...
void spell_plague(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if (saves_spell(level, victim, DAM_DISEASE) ||
        (IS_NPC(victim) && IS_SET(victim->act, ACT_UNDEAD)) || (IS_NPC(victim) && victim->pIndexData->vnum == MOB_VNUM_RAGER))
    {
        if (ch == victim)
...
void spell_poison(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if (target == TAR_OBJ_INV)
    {
        ...
        if (obj->item_type == ITEM_WEAPON)
        {
            if (IS_WEAPON_STAT(obj, WEAPON_FLAMING) || IS_WEAPON_STAT(obj, WEAPON_FROST) || IS_WEAPON_STAT(obj, WEAPON_VAMPIRIC) || IS_WEAPON_STAT(obj, WEAPON_SHARP) || IS_WEAPON_STAT(obj, WEAPON_VORPAL) || IS_WEAPON_STAT(obj, WEAPON_SHOCKING) || IS_OBJ_STAT(obj, ITEM_BLESS) || IS_OBJ_STAT(obj, ITEM_BURN_PROOF))
            {
...
void spell_sleep(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if (IS_AFFECTED(victim, AFF_SLEEP) || (IS_NPC(victim) && IS_SET(victim->act, ACT_UNDEAD)) || (level + 2) < victim->level || saves_spell(level - 1, victim, DAM_CHARM) || (!(str_cmp(race_table[victim->race].name, "lich"))))
    {
...
void spell_slow(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if ((victim != ch) &&
        (saves_spell(level, victim, DAM_OTHER) || IS_SET(victim->imm_flags, IMM_MAGIC)))
    {
        if (victim != ch)
            send_to_char("Nothing seemed to happen.\n\r", ch);
...
void spell_teleport(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if (victim->in_room == NULL || (!IS_NPC(ch) && IS_SET(victim->in_room->room_flags, ROOM_NO_RECALL)) || (victim != ch && IS_SET(victim->imm_flags, IMM_SUMMON)) || (!IS_NPC(ch) && victim->fighting != NULL) || (victim != ch))
    {
        send_to_char("You failed.\n\r", ch);
        return;
    }
...
void spell_turn_undead(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    else
    {
        ...
        for (victim = ch->in_room->people; victim != NULL; victim = v_next)
        {
            ...
            if ((ch->level * 3 < count * 2) || (ch->level < 20 && num > 0) || (ch->level < 40 && num > 1) || (ch->level < 51 && num > 1) || (ch->level < (victim->level + 3)) || saves_spell(ch->level, victim, DAM_OTHER) || !IS_NPC(victim))
            {
...
void spell_word_of_recall(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if ((IS_SET(victim->in_room->room_flags, ROOM_NO_RECALL) && !IS_SET(victim->in_room->room_flags, ROOM_CONSECRATED)) || IS_AFFECTED(victim, AFF_CURSE) || is_affected(victim, gsn_aristaeia))
    {
        send_to_char("Spell failed.\n\r", victim);
        return;
    }
...
void spell_windwall(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    for (vch = ch->in_room->people; vch != NULL; vch = vch_next)
    {
        ...
        if ((number_range(0, 1) == 0) && !saves_spell(level, vch, DAM_BASH) && !is_affected(vch, check))
        {
...
void spell_summon_fire_elemental(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    {
        if (IS_NPC(check))
            if ((check->master == ch) && ((check->pIndexData->vnum == MOB_VNUM_FIRE_ELEMENTAL) || (check->pIndexData->vnum == MOB_VNUM_AIR_ELEMENTAL) || (check->pIndexData->vnum == MOB_VNUM_EARTH_ELEMENTAL) || (check->pIndexData->vnum == MOB_VNUM_WATER_ELEMENTAL)))
                count++;
    }
    if ((count == 1 && ch->level < 30) || (count == 2 && ch->level < 37) || (count == 4))
    {
...
void spell_summon_earth_elemental(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if ((count == 1 && ch->level < 30) || (count == 2 && ch->level < 37) || (count == 4) || (count == 1 && level == 70))
    {
        send_to_char("You already control as many elementals as you can.\n\r", ch);
        return;
    }
...
void spell_summon_air_elemental(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    {
        if (IS_NPC(check))
            if ((check->leader == ch) && ((check->pIndexData->vnum == MOB_VNUM_FIRE_ELEMENTAL) || (check->pIndexData->vnum == MOB_VNUM_AIR_ELEMENTAL) || (check->pIndexData->vnum == MOB_VNUM_EARTH_ELEMENTAL) || (check->pIndexData->vnum == MOB_VNUM_WATER_ELEMENTAL)))
                count++;
    }
    if ((count == 1 && ch->level < 30) || (count == 2 && ch->level < 37) || (count == 4))
    {
...
void spell_earthmaw(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    int dam, save_num;
    int count;
    if ((ch->in_room->sector_type == SECT_AIR) || (ch->in_room->sector_type == SECT_WATER_SWIM) || (ch->in_room->sector_type == SECT_WATER_NOSWIM))
    {
...
void spell_consecrate(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    {
        if ((pexit = in_room->exit[door]) == NULL || (room_check = pexit->u1.to_room) == NULL)
            continue;
...
void spell_brew(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if ((obj->pIndexData->vnum == 4406) || (obj->pIndexData->vnum == 4406))
    {
...
void spell_familiar(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    {
        if (IS_NPC(check))
            if ((check->master == ch) && (check->pIndexData->vnum == MOB_VNUM_UNICORN || check->pIndexData->vnum == MOB_VNUM_DRAGON))
            {
    ...
    if (number_percent() < level)
    {
        for (obj_lose = victim->carrying; obj_lose != NULL; obj_lose = obj_next)
        {
            ...
            default:
                continue;
            case ITEM_SCROLL:
                msg = "$p bursts into flames and turns to ash.";
                break;
            case ITEM_POTION:
                msg = "$p bubbles and vaporises.";
                break;
            }
...
void spell_shadowself(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    for (check = char_list; check != NULL; check = check->next)
    {
        if (IS_NPC(check) && check->pIndexData->vnum == MOB_VNUM_SHADOW && (!str_cmp(check->name, ch->name)))
        {
...
void spell_bless_weapon(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    */
    ...
    case (0):
        ...
        break;
    case (1):
        type_name = "Lightbringer";
        break;
    case (2):
        type_name = "Shadowbane";
        break;
    }
...
void spell_revolt(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    for (charmie = ch->in_room->people; charmie != NULL; charmie = charmie_next)
    {
        ...
        if (!IS_AFFECTED(charmie, AFF_CHARM) || charmie->leader != victim || saves_spell(level, victim, DAM_OTHER))
            continue;
...
void spell_safety(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    for (gch = ch->in_room->people; gch != NULL; gch = gch_next)
    {
        ...
        if (IS_SET(gch->in_room->room_flags, ROOM_NO_RECALL) ||
            IS_AFFECTED(gch, AFF_CURSE) || (ch->in_room->cabal != 0 && ch->cabal != ch->in_room->cabal))
        {
            send_to_char("Spell failed.\n\r", gch);
        }
        else
...
void spell_nightwalk(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    chance = (level + 30);
    if (number_percent() > chance || pRoomIndex == NULL)
    {
...
void spell_alarm(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if (pRoomIndex->cabal != 0 && pRoomIndex->cabal != ch->cabal)
    {
...
bool check_spellcraft(CHAR_DATA *ch, int sn)
{
    ...
    if (sn == skill_lookup("windwall") || sn == skill_lookup("tsunami"))
        chance = 0;

    if (sn == skill_lookup("fireball") || sn == skill_lookup("iceball") || sn == skill_lookup("cone of cold"))
        chance -= 12;

    if (sn == skill_lookup("acid blast") || sn == skill_lookup("concatenate"))
        chance -= 10;

    if (sn == skill_lookup("magic missle") || sn == skill_lookup("chill touch") || sn == skill_lookup("firestream") || sn == skill_lookup("color spray"))
        chance += 10;
...
void spell_animate_object(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    for (check = char_list; check != NULL; check = check->next)
    {
        if (!IS_NPC(check) || check->master != ch)
            continue;
        if (check->pIndexData->vnum == MOB_VNUM_ARMOR || check->pIndexData->vnum == MOB_VNUM_WEAPON || check->pIndexData->vnum == MOB_VNUM_KEY)
            found = TRUE;
    ...
    type = obj->item_type;
    if (type != ITEM_WEAPON && type != ITEM_ARMOR && type != ITEM_KEY && type != ITEM_FURNITURE)
    {
        send_to_char("You can't animate that kind of object.\n\r", ch);
        return;
    }
    if (type == ITEM_ARMOR && (!IS_SET(obj->wear_flags, ITEM_WEAR_BODY) && !IS_SET(obj->wear_flags, ITEM_WEAR_HANDS) && !IS_SET(obj->wear_flags, ITEM_WEAR_SHIELD)))
    {
    ...
    if (type == ITEM_ARMOR)
    {
        ...
        if (IS_SET(obj->wear_flags, ITEM_WEAR_HANDS) || IS_SET(obj->wear_flags, ITEM_WEAR_SHIELD))
    ...
    else
    {
        ...
        if (type == ITEM_WEAPON)
        {
            ...
            {
            case WEAPON_EXOTIC:
                mob->material = str_dup("exotic");
                break;
            case WEAPON_SWORD:
                mob->material = str_dup("sword");
                break;
            case WEAPON_DAGGER:
                mob->material = str_dup("dagger");
                break;
            case WEAPON_SPEAR:
                mob->material = str_dup("spear");
                break;
            case WEAPON_MACE:
                mob->material = str_dup("mace");
                break;
            case WEAPON_AXE:
                mob->material = str_dup("axe");
                break;
            case WEAPON_FLAIL:
                mob->material = str_dup("flail");
                break;
            case WEAPON_WHIP:
                mob->material = str_dup("whip");
                break;
            case WEAPON_POLEARM:
                mob->material = str_dup("polearm");
                break;
            case WEAPON_STAFF:
                mob->material = str_dup("staff");
                break;
            case WEAPON_HAND:
                mob->material = str_dup("hands");
                break;
            }
...
void spell_transfer_object(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if (victim->carry_number >= can_carry_n(victim) || victim->carry_weight >= can_carry_w(victim) || obj->pIndexData->vnum == OBJ_VNUM_ARCANA || obj->pIndexData->vnum == OBJ_VNUM_RAGER || obj->pIndexData->vnum == OBJ_VNUM_ANCIENT || obj->pIndexData->vnum == OBJ_VNUM_KNIGHT || obj->pIndexData->vnum == OBJ_VNUM_OUTLAW || obj->pIndexData->vnum == OBJ_VNUM_BOUNTY || obj->pIndexData->vnum == OBJ_VNUM_EMPIRE || obj->pIndexData->vnum == OBJ_VNUM_ENFORCER)
    {
...
void spell_disintegrate(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if (saves_spell(level - 1, victim, DAM_ENERGY) || saves_spell(level - 4, victim, DAM_ENERGY) || saves_spell(level + 12, victim, DAM_ENERGY))
    {
    ...
    for (obj = victim->carrying; obj != NULL; obj = obj_next)
    {
        ...
        if (obj->pIndexData->limtotal == 1)
            obj_save += 15;
        else if (obj->pIndexData->limtotal <= 3 && obj->pIndexData->limtotal > 0)
            obj_save += 10;
        else if (obj->pIndexData->limtotal <= 10 && obj->pIndexData->limtotal > 0)
            obj_save += 5;
        else if (obj->pIndexData->limtotal <= 15 && obj->pIndexData->limtotal > 0)
            obj_save += 3;
...
void spell_power_word_fear(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if (IS_NPC(victim))
    {
        if (victim->pIndexData->vnum == MOB_VNUM_ANCIENT || victim->pIndexData->vnum == MOB_VNUM_BOUNTY || victim->pIndexData->vnum == MOB_VNUM_RAGER || victim->pIndexData->vnum == MOB_VNUM_OUTLAW || victim->pIndexData->vnum == MOB_VNUM_KNIGHT || victim->pIndexData->vnum == MOB_VNUM_ARCANA || victim->pIndexData->vnum == MOB_VNUM_ENFORCER)
        {
            act("$n shivers momentarily but it passes.", victim, 0, 0, TO_ROOM);
...
void spell_restoration(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if ((is_affected(victim, skill_lookup("rot"))) && check_dispel(level, victim, skill_lookup("rot")))
    {
...
void spell_confuse(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    for (rch = ch->in_room->people; rch; rch = rch->next_in_room)
    {
        ...
        if (rch != ch && can_see(ch, rch) && number_range(1, count) == 1)
            break;
    }
