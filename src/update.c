...
int mana_gain( CHAR_DATA *ch )
{
	...
	switch (get_curr_stat(ch,STAT_WIS)) {
		case 25: gain += 7 * gain / 20; break;
		case 24: gain += 6 * gain / 20; break;
		case 23: gain += 5 * gain / 20; break;
		case 22: gain += 4 * gain / 20; break;
		case 21: gain += 3 * gain / 20; break;
		case 20: gain += 2 * gain / 20; break;
		case 19: gain += 1 * gain / 20; break;
...
void obj_update( void )
{
	...
	if (obj->item_type == ITEM_CORPSE_NPC && obj->contains) {
		...
		if (obj->contains) {
			...
			else act("The contents of $p spill from your hands onto the ground as it decays.",obj->carried_by,obj,0,TO_CHAR);
...
void updateRaceAff(CHAR_DATA *ch)
{
	...
	// Flight
	if (ch->race == race_lookup("cloud") || ch->race == race_lookup("draconian") || ch->race == race_lookup("ethereal")
	|| ch->race == race_lookup("arial") || ch->race == race_lookup("illithid"))
	{
...
void resurrect(CHAR_DATA *ch, AFFECT_DATA *paf) {
	...
	if ( ch->position < skill_table[paf->type].minimum_position ) return send_to_char("Your resurrection has been interrupted!\n\r",ch);
	...
	if (!corpse) return send_to_char("The corpse is no longer here. You cannot resurrect it.\n\r",ch);
	if ((victim = find_char_by_name(corpse->owner)) == NULL) return send_to_char("They no longer seem to exist.\n\r",ch);
	if (victim->ghost == 0) return send_to_char("They are already living, it seems.\n\r",ch);
