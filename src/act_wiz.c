...
void do_induct(CHAR_DATA *ch, char *argument)
{
	...
	if (!str_prefix(arg2, "none"))
	{
		...
		else
		{
			...
			for (gns = 0; gns < MAX_SKILL; gns++)
			{
				...
				if (skill_table[sn].skill_level[victim->class] < LEVEL_HERO + 1 && victim->pcdata->learned[sn] > 0)
	...
	if (!(cabal_restr_table[cabal].acc_align & iAlign) && !IS_IMMORTAL(victim))
	{
		...
		{
		case ALIGN_E:
			send_to_char("That cabal does not accept evil characters.\n\r", ch);
			break;
		case ALIGN_N:
			send_to_char("That cabal does not accept neutral characters.\n\r", ch);
			break;
		case ALIGN_G:
			send_to_char("That cabal does not accept good characters.\n\r", ch);
			break;
		default:
			send_to_char("ERROR: SEEK AN IMPLEMENTOR!\n\r", ch);
		}
		return;
	}
	...
	if (!(cabal_restr_table[cabal].acc_ethos & iEthos) && !IS_IMMORTAL(victim))
	{
		...
		{
		case ETHOS_C:
			send_to_char("That cabal does not accept chaotic characters.\n\r", ch);
			break;
		case ETHOS_N:
			send_to_char("That cabal does not accept neutral characters.\n\r", ch);
			break;
		case ETHOS_L:
			send_to_char("That cabal does not accept lawful characters.\n\r", ch);
			break;
		default:
			send_to_char("ERROR: SEEK AN IMPLEMENTOR!\n\r", ch);
		}
...
void do_leaderpeek(CHAR_DATA *ch, char *argument)
{
	...
	if ((ch->level < 56 && ch->pcdata->induct != CABAL_LEADER && ch->pcdata->empire != EMPIRE_EMPEROR))
	{
		send_to_char("Huh?\n\r", ch);
		return;
	}
...
void do_outfit(CHAR_DATA *ch, char *argument)
{
	...
	if (((obj = get_eq_char(ch, WEAR_WIELD)) == NULL || !IS_WEAPON_STAT(obj, WEAPON_TWO_HANDS)) && (obj = get_eq_char(ch, WEAR_LIGHT)) == NULL && (obj = get_eq_char(ch, WEAR_SHIELD)) == NULL)
	{
...
void do_ostat(CHAR_DATA *ch, char *argument)
{
	...
	sprintf(buf,
			...
			obj->carried_by == NULL ? "(none)" : can_see(ch, obj->carried_by) ? obj->carried_by->name
																			  : "someone",
			obj->wear_loc);
	send_to_char(buf, ch);
	...
	sprintf(buf, "COUNT_DATA: Limit is %d, Count is %d -->%s.\n\r", obj->pIndexData->limtotal, obj->pIndexData->limcount,
			(obj->pIndexData->limcount < obj->pIndexData->limtotal) ? "Not maxxed" : (obj->pIndexData->limtotal == 0) ? "Non-limited"
																													  : "Maxxed");
	send_to_char(buf, ch);
...
void do_mstat(CHAR_DATA *ch, char *argument)
{
	...
	for (paf = victim->affected; paf != NULL; paf = paf->next)
	{
		...
		sprintf(buf,
				...
				paf->duration, paf->where == TO_IMMUNE ? "imm" : paf->where == TO_RESIST ? "res"
															 : paf->where == TO_VULN	 ? "vuln"
																						 : "aff",
				paf->where == TO_IMMUNE || paf->where == TO_RESIST || paf->where == TO_VULN ? imm_bit_name(paf->bitvector) : affect_bit_name(paf->bitvector),
...
void do_mwhere(CHAR_DATA *ch, char *argument)
{
	...
	if (argument[0] == '\0')
	{
		...
		for (d = descriptor_list; d != NULL; d = d->next)
		{
			if (d->character != NULL && d->connected == CON_PLAYING && d->character->in_room != NULL && can_see(ch, d->character) && can_see_room(ch, d->character->in_room))
			{
				victim = d->character;
				count++;
				if (d->original != NULL)
	...
	{
		if (victim->in_room != NULL && can_see(ch, victim) && is_name(argument, IS_NPC(victim) ? victim->name : victim->original_name))
		{
			found = TRUE;
			count++;
...
{
	if (IS_TRUSTED(ch, GOD) || (IS_TRUSTED(ch, IMMORTAL) && obj->level <= 20 && obj->cost <= 1000) || (IS_TRUSTED(ch, DEMI) && obj->level <= 10 && obj->cost <= 500) || (IS_TRUSTED(ch, ANGEL) && obj->level <= 5 && obj->cost <= 250) || (IS_TRUSTED(ch, AVATAR) && obj->level == 0 && obj->cost <= 100))
		return TRUE;
	else
		return FALSE;
}
...
void do_purge(CHAR_DATA *ch, char *argument)
{
	...
	if (arg[0] == '\0')
	{
		...
		for (victim = ch->in_room->people; victim != NULL; victim = vnext)
		{
			...
			if (IS_NPC(victim) && !IS_SET(victim->act, ACT_NOPURGE) && victim != ch /* safety precaution */ && !IS_AFFECTED(victim, AFF_CHARM))
...
void do_sockets(CHAR_DATA *ch, char *argument)
{
	...
	for (d = descriptor_list; d != NULL; d = d->next)
	{
		if (d->character != NULL && can_see(ch, d->character) && (arg[0] == '\0' || is_name(arg, d->character->name) || (d->original && is_name(arg, d->original->name))))
		{
			count++;

			sprintf(buf + strlen(buf), "[%3d %18s %4d %5s %s] %-15s %s@%s\n\r",
					d->descriptor,
					d->connected == (CON_PLAYING) ? "Playing" : d->connected == (CON_GET_NAME)			 ? "Getting name"
															...
															: d->connected == (CON_GET_CABAL)			 ? "Getting cabal"
																										 : "null",
					...
					d->original ? (d->original->original_name) ? d->original->original_name : d->original->name : d->character ? (d->character->original_name) ? d->character->original_name : d->character->name
																															   : "(none)",
...
void do_multicheck(CHAR_DATA *ch, char *argument)
{
	...
	for (j = 0; j < i - 1; j++)
	{
							   ...
							   get_end_host(CHARLIST[j - 1].des->host))))
		{
			...
			sprintf(buf + strlen(buf), "[%3d %2d] %s@%s\n\r",
					...
					CHARLIST[j].des->original ? CHARLIST[j].des->original->original_name ? CHARLIST[j].des->original->original_name : CHARLIST[j].des->original->name : CHARLIST[j].des->character ? CHARLIST[j].des->character->original_name ? CHARLIST[j].des->character->original_name : CHARLIST[j].des->character->name
																																																   : "(none)",
					CHARLIST[j].des->host);
		}
	}
				...
				get_end_host(CHARLIST[j - 1].des->host)))
	{
		...
		sprintf(buf + strlen(buf), "[%3d %2d] %s@%s\n\r",
				...
				CHARLIST[j].des->original ? CHARLIST[j].des->original->name : CHARLIST[j].des->character ? CHARLIST[j].des->character->name
																										 : "(none)",
				CHARLIST[j].des->host);
	}
	...
	if (ch->in_room->affected != NULL)
	{
		...
		for (paf = ch->in_room->affected; paf != NULL; paf = paf->next)
		{
			...
			sprintf(buf, "modifies %s by %d for %d hours with %s-bits %s, owner %s, level %d.\n\r",
					...
					paf->modifier,
					paf->duration,
					paf->where == TO_ROOM_FLAGS ? "flag" : paf->where == TO_ROOM_CONST ? "const"
																					   : "aff",
					"NULL",
...
void do_ghost(CHAR_DATA *ch, char *argument)
{
	...
	if (!str_cmp(arg1, "zone"))
	{
		for (d = descriptor_list; d; d = d->next)
		{
			if (d->connected == CON_PLAYING && d->character != ch && d->character->in_room != NULL && can_see(ch, d->character) && d->character->in_room->area == ch->in_room->area)
			{
				if (!str_cmp(arg2, "yes"))
				{
					d->character->ghost = 15;
...
void do_otype(CHAR_DATA *ch, char *argument)
{
	...
	if ((!str_cmp(arg1, "armor") && !str_cmp(arg1, "weapon") && arg2[0] != '\0'))
	{
	...
	for (; vnum <= top_vnum_obj; vnum++)
	{
		...
		{
			if ((obj->item_type == type && type2 == 0 && str_cmp(arg1, "weapon") && str_cmp(arg1, "armor")) || (obj->item_type == type && obj->value[0] == type2 && str_cmp(arg1, "armor")) || (obj->item_type == type && IS_SET(obj->wear_flags, type2) && str_cmp(arg1, "weapon")))
			{
...
void do_wflag(CHAR_DATA *ch, char *argument)
{
	...
	case 4:
		...
		break;
	case 5:
		bitvector = WEAPON_VORPAL;
		break;
	case 6:
		bitvector = WEAPON_TWO_HANDS;
		break;
	case 7:
		...
		for (parea = area_first; parea != NULL; parea = parea->next)
		{
			...
			for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
			{
					 ...
					 from_room = from_room->next)
				{
					 ...
					 */
					if (from_room->vnum < parea->min_vnum || from_room->vnum > parea->max_vnum)
						continue;
					...
					for (door = 0; door < 5; door++)
					{
						...
						if ((pexit = from_room->exit[door]) != NULL)
						{
							 ...
							 */
							if (to_room != NULL && (to_room->vnum < parea->min_vnum || to_room->vnum > parea->max_vnum))
							{
								found = TRUE;
...
void do_info(CHAR_DATA *ch, char *argument)
{
	...
	else if (!str_cmp(arg, "rflag"))
	{
		...
		for (vnum = low; vnum <= high; vnum++)
			...
			{
				sprintf(buf, "[%5ld] %s\n\r",
						pRoomIndex->vnum, flag_string(room_flags, pRoomIndex->room_flags));
				add_buf(buffer, buf);
			}
	}
	else
...
void show_alist_free(CHAR_DATA *ch, char *argument)
{
	...
	while (--i >= 0)
	{
		...
		{
			sprintf(buf, "[%-5ld-%5ld] %5ld\n\r", areas[i]->max_vnum + 1, areas[i - 1]->min_vnum - 1, (((areas[i - 1]->min_vnum - 1) - (areas[i]->max_vnum + 1)) + 1));
			add_buf(output, buf);
		}
	}
