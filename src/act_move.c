...
void move_char(CHAR_DATA *ch, int door, bool follow)
{
	...
	in_room = ch->in_room;
	if ((pexit = in_room->exit[door]) == NULL || pexit->u1.vnum == 0 || (to_room = pexit->u1.to_room) == NULL || !can_see_room(ch, pexit->u1.to_room))
	{
	...
	if (IS_ROOM_AFFECTED(in_room, AFF_ROOM_RANDOMIZER))
	{
		...
		{
			d0 = number_range(0, 5);
			if ((pexit = in_room->exit[d0]) == NULL || (to_room = pexit->u1.to_room) == NULL || !can_see_room(ch, pexit->u1.to_room))
				continue;
	...
	if (IS_SET(pexit->exit_info, EX_CLOSED) && (!IS_AFFECTED(ch, AFF_PASS_DOOR) || IS_SET(pexit->exit_info, EX_NOPASS)) && !IS_TRUSTED(ch, ANGEL))
	{
	...
	if (!IS_NPC(ch))
	{
		...
		if ((in_room->sector_type == SECT_WATER_NOSWIM || to_room->sector_type == SECT_WATER_NOSWIM) && (!IS_AFFECTED(ch, AFF_FLYING) && !IS_AFFECTED(ch, AFF_WATERBREATHING)))
		{
			OBJ_DATA *obj;
			bool found;
	...
	if (ch->invis_level < LEVEL_HERO && !is_affected(ch, gsn_creep) && (!IS_AFFECTED(ch, AFF_SNEAK) && get_skill(ch, skill_lookup("shadowstep")) <= 90))
	{
		un_hide(ch, NULL);
	...
	if ((to_room->sector_type != SECT_FOREST) && (to_room->sector_type != SECT_MOUNTAIN) && (to_room->sector_type != SECT_HILLS) && (to_room->sector_type != SECT_FIELD) && (to_room->sector_type != SECT_DESERT))
	{
		un_camouflage(ch, NULL);
	}
	...
	if (!IS_AFFECTED(ch, AFF_SNEAK) && ch->invis_level < LEVEL_HERO && !is_affected(ch, gsn_creep) && get_skill(ch, skill_lookup("shadowstep")) <= 90)
	{
		if (is_affected(ch, gsn_door_bash))
...
void do_rest(CHAR_DATA *ch, char *argument)
{
	...
	if (obj != NULL)
	{
		if (!IS_SET(obj->item_type, ITEM_FURNITURE) || (!IS_SET(obj->value[2], REST_ON) && !IS_SET(obj->value[2], REST_IN) && !IS_SET(obj->value[2], REST_AT)))
		{
...
void do_sit(CHAR_DATA *ch, char *argument)
{
	...
	if (obj != NULL)
	{
		if (!IS_SET(obj->item_type, ITEM_FURNITURE) || (!IS_SET(obj->value[2], SIT_ON) && !IS_SET(obj->value[2], SIT_IN) && !IS_SET(obj->value[2], SIT_AT)))
		{
...
void do_acute_vision(CHAR_DATA *ch, char *argument)
{
	...
	if (get_skill(ch, gsn_acute_vision) == 0 || ch->level < skill_table[gsn_acute_vision].skill_level[ch->class])
	{
...
void murder_entry_trigger(CHAR_DATA *ch)
{
	...
	for (rch = ch->in_room->people; rch != NULL; rch = rch_next)
	{
		...
		if (!IS_NPC(rch))
			continue;
		if (!IS_SET(rch->off_flags, SPAM_MURDER))
			continue;
		if (rch->fighting != NULL)
			continue;
		if (!can_see(rch, ch))
			continue;
		if (rch->last_fought != NULL)
			continue;
		if (is_safe(rch, ch))
			continue;
...
void do_awareness(CHAR_DATA *ch, char *argument)
{
	...
	if (chance == 0 || ch->level < skill_table[gsn_awareness].skill_level[ch->class])
	{
...
void do_door_bash(CHAR_DATA *ch, char *argument)
{
	...
	if (chance == 0 || ch->level < skill_table[gsn_door_bash].skill_level[ch->class])
	{
...
void move_prog_bones(CHAR_DATA *ch)
{
	...
	if (ch->in_room->vnum != 13868 && ch->in_room->vnum != 13869 && ch->in_room->vnum != 13871 && ch->in_room->vnum != 13872 && ch->in_room->vnum != 13884 && ch->in_room->vnum != 13873)
		return;
	...
	{
	case 1:
		sprintf(buf, "Die, thou who invades out rest!");
		break;
	case 2:
		sprintf(buf, "%s, you shall pay for your desecration of our bones!", ch->name);
		break;
	case 3:
		sprintf(buf, "Flesh...flesh for us at last...");
		break;
	case 4:
...
void mob_entry_wraith(CHAR_DATA *ch)
{
	...
	if (IS_NPC(ch))
		return;
	if (str_cmp(ch->in_room->area->name, "mythforest.are") || number_percent() > 4)
		return;
...
void do_animal_call(CHAR_DATA *ch, char *argument)
{
	...
	if ((chance = get_skill(ch, gsn_animal_call)) == 0 || ch->level < skill_table[gsn_animal_call].skill_level[ch->class])
	{
	...
	{
		if (IS_NPC(mob) && IS_AFFECTED(mob, AFF_CHARM) && (mob->master == ch) && ((mob->pIndexData->vnum == MOB_VNUM_FALCON) || (mob->pIndexData->vnum == MOB_VNUM_WOLF) || (mob->pIndexData->vnum == MOB_VNUM_BEAR) || (mob->pIndexData->vnum == MOB_VNUM_LION)))
			break;
	}
...
void do_forest_blending(CHAR_DATA *ch, char *argument)
{
	...
	if (chance < 5 || ch->level < skill_table[gsn_forest_blending].skill_level[ch->class])
	{
...
{
	if ((ch->in_room->sector_type == SECT_FIELD) || (ch->in_room->sector_type == SECT_FOREST) || (ch->in_room->sector_type == SECT_HILLS) || (ch->in_room->sector_type == SECT_MOUNTAIN) || (ch->in_room->sector_type == SECT_DESERT))
	{
		return TRUE;
	}
	return FALSE;
