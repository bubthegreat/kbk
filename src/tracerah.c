...
void do_legsweep(CHAR_DATA *ch, char *argument)
{
	...
	if ((chance = get_skill(ch, gsn_legsweep)) == 0 || (!IS_NPC(ch) && ch->level < skill_table[gsn_legsweep].skill_level[ch->class]))
	{
		send_to_char("Huh?\n\r", ch);
		return;
	}
...
bool can_live_in(CHAR_DATA *ch, int hometown)
{
	...
	if ((hometown_table[hometown].align == ALIGN_NONE) || (hometown_table[hometown].align == ALIGN_GN && IS_EVIL(ch)) || (hometown_table[hometown].align == ALIGN_NE && IS_GOOD(ch)) || (hometown_table[hometown].align == ALIGN_G && !IS_GOOD(ch)) || (hometown_table[hometown].align == ALIGN_N && !IS_NEUTRAL(ch)) || (hometown_table[hometown].align == ALIGN_E && !IS_EVIL(ch)) || (hometown_table[hometown].align == ALIGN_GE && IS_NEUTRAL(ch)))
		return FALSE;

	if ((hometown_table[hometown].ethos == ETHOS_NONE) || (hometown_table[hometown].ethos == ETHOS_LN && IS_CHAOTIC(ch)) || (hometown_table[hometown].ethos == ETHOS_NC && IS_LAWFUL(ch)) || (hometown_table[hometown].ethos == ETHOS_L && !IS_LAWFUL(ch)) || (hometown_table[hometown].ethos == ETHOS_N && !IS_ENEUTRAL(ch)) || (hometown_table[hometown].ethos == ETHOS_C && !IS_CHAOTIC(ch)) || (hometown_table[hometown].ethos == ETHOS_LC && IS_ENEUTRAL(ch)))
		return FALSE;
	...
	if (number_percent() < chance)
	{
		...
		if (!IS_NPC(ch) && !IS_NPC(victim) && (ch->fighting == NULL || victim->fighting == NULL))
		{
			sprintf(buf, "Help! %s just tripped me with %s weapon!",
					PERS(ch, victim), ch->sex == 0 ? "its" : ch->sex == 1 ? "his"
																		  : "her");
			do_myell(victim, buf);
		}
...
void do_barkskin(CHAR_DATA *ch, char *argument)
{
	...
	if ((get_skill(ch, gsn_barkskin) == 0) || (ch->level < skill_table[gsn_barkskin].skill_level[ch->class]))
	{
...
void do_pummel(CHAR_DATA *ch, char *argument)
{
	...
	if (get_eq_char(ch, WEAR_SHIELD) != NULL || get_eq_char(ch, WEAR_HOLD) != NULL || get_eq_char(ch, WEAR_LIGHT) != NULL || get_eq_char(ch, WEAR_DUAL_WIELD) != NULL || get_eq_char(ch, WEAR_WIELD) != NULL)
	{
...
void do_darkshout(CHAR_DATA *ch, char *argument)
{
	...
	if (devil == DARKSHOUT_MEPHISTOPHELES || devil == DARKSHOUT_BAALZEBUB || devil == DARKSHOUT_NECRUVIAN || devil == DARKSHOUT_NECRUVIAN || devil == DARKSHOUT_BAAL)
	{
		init_affect(&af);
		...
		{
		case DARKSHOUT_MEPHISTOPHELES:
			af.affect_list_msg = str_dup("grants a chance for an additional attack");
			break;
		case DARKSHOUT_BAAL:
			af.affect_list_msg = str_dup("relentlessly assaulting those with good hearts");
			break;
		case DARKSHOUT_NECRUVIAN:
			af.affect_list_msg = str_dup("siphoning the souls of good-aligned beings");
			break;
		case DARKSHOUT_BAALZEBUB:
			af.affect_list_msg = str_dup("prevents usage of darkshout");
			break;
		}
		af.aftype = AFT_SKILL;
		af.where = TO_AFFECTS;
...
void spell_charge_weapon(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	if (attack_table[obj->value[3]].damage != DAM_SLASH && attack_table[obj->value[3]].damage != DAM_PIERCE && attack_table[obj->value[3]].damage != DAM_BASH)
...
int count_hands(CHAR_DATA *ch)
{
	...
	if (get_eq_char(ch, WEAR_WIELD) != NULL)
	{
		...
		if ((weapon != NULL && ch->size < SIZE_LARGE && IS_WEAPON_STAT(weapon, WEAPON_TWO_HANDS)) || (weapon != NULL && weapon->value[0] == WEAPON_STAFF) || (weapon != NULL && weapon->value[0] == WEAPON_POLEARM) || (weapon != NULL && weapon->value[0] == WEAPON_SPEAR))
		{
